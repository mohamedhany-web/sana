...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
            <div>
                <p class="text-gray-500 font-medium mb-1">{{ __('الموظف') }}</p>
                <p class="font-bold text-gray-900">{{ $leave->employee->name }}</p>
                @if($leave->employee->employee_code)
                    <p class="text-gray-500 mt-1">{{ __('الرمز:') }} {{ $leave->employee->employee_code }}</p>
                @endif
                @if($leave->employee->employeeJob)
                    <p class="text-gray-500">{{ __('الوظيفة:') }} {{ $leave->employee->employeeJob->name }}</p>
                @endif
            </div>
            <div>
                <p class="text-gray-500 font-medium mb-1">{{ __('نوع الإجازة') }}</p>
                <p class="font-bold text-gray-900">{{ $leave->type_label }}</p>
            </div>
            <div>
                <p class="text-gray-500 font-medium mb-1">{{ __('من') }}</p>
                <p class="font-semibold text-gray-900">{{ $leave->start_date->format('Y-m-d') }}</p>
            </div>
            <div>
                <p class="text-gray-500 font-medium mb-1">{{ __('إلى') }}</p>
                <p class="font-semibold text-gray-900">{{ $leave->end_date->format('Y-m-d') }}</p>
            </div>
            <div>
                <p class="text-gray-500 font-medium mb-1">{{ __('عدد الأيام') }}</p>
                <p class="font-semibold text-gray-900">{{ $leave->days }} يوم</p>
            </div>
            <div>
                <p class="text-gray-500 font-medium mb-1">{{ __('تاريخ التقديم') }}</p>
                <p class="font-semibold text-gray-900">{{ $leave->created_at->format('Y-m-d H:i') }}</p>
            </div>
            <div class="md:col-span-2">
                <p class="text-gray-500 font-medium mb-1">{{ __('السبب') }}</p>
    ...
    @if($leave->status !== 'pending' && $leave->reviewer)
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <h2 class="text-xl font-bold text-gray-900 mb-4">{{ __('المراجعة') }}</h2>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                <div>
                    <p class="text-gray-500">{{ __('بواسطة') }}</p>
                    <p class="font-semibold">{{ $leave->reviewer->name }}</p>
                </div>
                <div>
                    <p class="text-gray-500">{{ __('التاريخ') }}</p>
                    <p class="font-semibold">{{ $leave->reviewed_at?->format('Y-m-d H:i') }}</p>
                </div>
                @if($leave->admin_notes)
                    <div class="md:col-span-2">
                        <p class="text-gray-500 mb-1">{{ __('ملاحظات') }}</p>
    ...
    @if($leave->status === 'pending')
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <h2 class="text-xl font-bold text-gray-900 mb-6">{{ __('قرار الموارد البشرية') }}</h2>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <form action="{{ route('employee.hr.leaves.approve', $leave) }}" method="POST">
                    @csrf
                    <div class="space-y-3">
                        <label class="block text-sm font-medium text-gray-700">{{ __('ملاحظات (اختياري)') }}</label>
                ...
                <form action="{{ route('employee.hr.leaves.reject', $leave) }}" method="POST">
                    @csrf
                    <div class="space-y-3">
                        <label class="block text-sm font-medium text-gray-700">{{ __('سبب الرفض') }} <span class="text-rose-600">*</span></label>
