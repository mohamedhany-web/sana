...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
        <div class="flex flex-wrap justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-gray-600 mt-1">{{ $employee->employeeJob?->name ?? __('بدون وظيفة محددة') }}</p>
            </div>
            @if($employee->is_active)
                <span class="h-fit text-xs font-bold px-3 py-1 rounded-full bg-emerald-100 text-emerald-800">{{ __('نشط') }}</span>
            @else
                <span class="h-fit text-xs font-bold px-3 py-1 rounded-full bg-gray-200 text-gray-700">{{ __('موقوف') }}</span>
            @endif
        </div>

        <dl class="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
            <div><dt class="text-gray-500 font-medium">{{ __('البريد') }}</dt><dd class="font-medium text-gray-900">{{ $employee->email }}</dd></div>
            <div><dt class="text-gray-500 font-medium">{{ __('الهاتف') }}</dt><dd class="font-medium text-gray-900">{{ $employee->phone ?: '—' }}</dd></div>
            <div><dt class="text-gray-500 font-medium">{{ __('رمز الموظف') }}</dt><dd class="font-medium text-gray-900">{{ $employee->employee_code ?: '—' }}</dd></div>
            <div><dt class="text-gray-500 font-medium">{{ __('تاريخ التوظيف') }}</dt><dd class="font-medium text-gray-900">{{ $employee->hire_date ? $employee->hire_date->format('Y-m-d') : '—' }}</dd></div>
            <div><dt class="text-gray-500 font-medium">{{ __('إنهاء الخدمة') }}</dt><dd class="font-medium text-gray-900">{{ $employee->termination_date ? $employee->termination_date->format('Y-m-d') : '—' }}</dd></div>
            <div><dt class="text-gray-500 font-medium">{{ __('الراتب (مرجعي)') }}</dt><dd class="font-medium text-gray-900 tabular-nums">{{ $employee->salary !== null ? number_format((float) $employee->salary, 2) : '—' }}</dd></div>
        </dl>

        @if($employee->employee_notes)
            <div class="mt-6">
                <p class="text-xs font-bold text-gray-500 mb-1">{{ __('ملاحظات إدارية (من الملف)') }}</p>
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        <ul class="divide-y divide-gray-100 max-h-64 overflow-y-auto text-sm">
            ...
            @empty
                <li class="px-5 py-8 text-center text-gray-500">{{ __('لا توجد طلبات.') }}</li>
            @endforelse
        </ul>
    </div>

    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
        <h3 class="text-lg font-bold text-gray-900 mb-4">{{ __('سجل الموارد البشرية') }}</h3>
        <p class="text-xs text-gray-500 mb-4">{{ __('تسجيلات داخلية (إنذارات، تقدير، ملاحظات) — تظهر هنا فقط لمن لديهم صلاحية HR.') }}</p>
        ...
        <form method="POST" action="{{ route('employee.hr.employees.hr-events.store', $employee) }}" class="space-y-3 mb-8 pb-8 border-b border-gray-100">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                    <label class="block text-xs font-semibold text-gray-600 mb-1">{{ __('النوع') }}</label>
                ...
                <div>
                    <label class="block text-xs font-semibold text-gray-600 mb-1">{{ __('تاريخ الحدث') }}</label>
            ...
            <div>
                <label class="block text-xs font-semibold text-gray-600 mb-1">{{ __('عنوان مختصر (اختياري)') }}</label>
            ...
            <div>
                <label class="block text-xs font-semibold text-gray-600 mb-1">{{ __('التفاصيل') }} <span class="text-rose-600">*</span></label>
            ...
            <button type="submit" class="px-4 py-2 rounded-lg bg-gray-900 hover:bg-gray-800 text-white text-sm font-bold">{{ __('إضافة للسجل') }}</button>
        </form>
        ...
        <ul class="space-y-4">
            ...
            @empty
                <li class="text-center text-gray-500 py-6">{{ __('لا توجد تسجيلات بعد.') }}</li>
            @endforelse
        </ul>
    </div>
</div>
