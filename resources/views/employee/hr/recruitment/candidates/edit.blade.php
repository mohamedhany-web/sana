...
<div class="max-w-2xl space-y-6">
    <a href="{{ route('employee.hr.recruitment.candidates.show', $candidate) }}" class="text-sm font-semibold text-gray-600 hover:text-gray-900"><i class="fas fa-arrow-right ml-1"></i> {{ __('العرض') }}</a>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6">
        <form method="POST" action="{{ route('employee.hr.recruitment.candidates.update', $candidate) }}" enctype="multipart/form-data" class="space-y-4">
            ...
            <div>
                <label class="block text-xs font-semibold text-gray-600 mb-1">{{ __('رابط معرض أعمال') }}</label>
            ...
            <div>
                <label class="block text-xs font-semibold text-gray-600 mb-1">{{ __('استبدال السيرة (اختياري)') }}</label>
                <input type="file" name="cv" accept=".pdf,.doc,.docx" class="w-full text-sm">
                @if($candidate->cv_path)
                    <p class="text-xs text-gray-500 mt-1">{{ __('الملف الحالي:') }} <a href="{{ $candidate->cvUrl() }}" target="_blank" class="text-violet-700 underline">{{ __('تحميل') }}</a></p>
                @endif
            </div>
            <div>
