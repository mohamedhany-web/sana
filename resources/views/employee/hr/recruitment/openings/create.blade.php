...
<div class="max-w-3xl space-y-6">
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6">
        <form method="POST" action="{{ route('employee.hr.recruitment.openings.store') }}" class="space-y-4">
            ...
            <div>
                <label class="block text-xs font-semibold text-gray-600 mb-1">{{ __('إغلاق التقديم (اختياري)') }}</label>
            ...
            <div class="flex gap-2 pt-2">
                ...
                <a href="{{ route('employee.hr.recruitment.openings.index') }}" class="px-5 py-2.5 rounded-lg bg-gray-200 text-gray-800 font-semibold text-sm">{{ __('إلغاء') }}</a>
            </div>
        </form>
    </div>
</div>
