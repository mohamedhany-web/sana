...
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:flex-wrap sm:items-center sm:justify-between gap-4">
        <p class="text-sm text-gray-600 max-w-2xl">{{ __('متابعة الموظفين، التوظيف والمقابلات، مراجعة الإجازات، وسجل HR. يمكن للإدارة أيضاً استخدام لوحة الإدارة للإعدادات المتقدمة.') }}</p>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        ...
        <div class="rounded-2xl border-2 border-slate-200/60 bg-gradient-to-br from-white to-slate-50/80 p-5 shadow-sm">
            <p class="text-sm font-semibold text-gray-600 mb-1">{{ __('موقوفون') }}</p>
        ...
        <div class="rounded-2xl border-2 border-rose-200/60 bg-gradient-to-br from-white to-rose-50/80 p-5 shadow-sm">
            <p class="text-sm font-semibold text-gray-600 mb-1">{{ __('إجازات معلّقة') }}</p>
        ...
        <div class="rounded-2xl border-2 border-emerald-200/60 bg-gradient-to-br from-white to-emerald-50/80 p-5 shadow-sm">
            <p class="text-sm font-semibold text-gray-600 mb-1">{{ __('موافق عليها (الشهر)') }}</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div class="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
            <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
                <h2 class="text-base font-bold text-gray-900">{{ __('إجازات تحتاج مراجعة') }}</h2>
                <a href="{{ route('employee.hr.leaves.index', ['status' => 'pending']) }}" class="text-xs font-bold text-rose-700 hover:underline">{{ __('الكل') }}</a>
            </div>
            <div class="overflow-x-auto max-h-80 overflow-y-auto">
                <table class="min-w-full text-sm">
                    <thead class="bg-gray-50 text-gray-600 font-semibold sticky top-0">
                        <tr>
                            <th class="text-right px-4 py-2">{{ __('الموظف') }}</th>
                            <th class="text-right px-4 py-2">{{ __('النوع') }}</th>
                            <th class="text-right px-4 py-2">{{ __('الفترة') }}</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50/80">
                            ...
                            <td class="px-4 py-2">
                                <a href="{{ route('employee.hr.leaves.show', $leave) }}" class="text-rose-700 font-bold hover:underline text-xs">{{ __('مراجعة') }}</a>
                            </td>
                        </tr>
                        @empty
                        <tr><td colspan="4" class="px-4 py-10 text-center text-gray-500">{{ __('لا توجد طلبات معلّقة.') }}</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
        ...
        <div class="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
            <div class="px-5 py-4 border-b border-gray-100">
                <h2 class="text-base font-bold text-gray-900">{{ __('إجازات معتمدة قادمة') }}</h2>
                <p class="text-xs text-gray-500 mt-1">{{ __('حتى نهاية الفترة المعتمدة ≥ اليوم') }}</p>
            </div>
            <ul class="divide-y divide-gray-100 max-h-80 overflow-y-auto text-sm">
                ...
                @empty
                <li class="px-5 py-10 text-center text-gray-500">{{ __('لا توجد إجازات معتمدة في الأفق.') }}</li>
                @endforelse
            </ul>
        </div>
    </div>
    ...
    <div class="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
            <h2 class="text-base font-bold text-gray-900">{{ __('تعيينات حديثة (آخر 6 أشهر)') }}</h2>
            <a href="{{ route('employee.hr.employees.index') }}" class="text-xs font-bold text-indigo-700 hover:underline">{{ __('الدليل') }}</a>
        </div>
        <ul class="divide-y divide-gray-100">
            ...
            @empty
            <li class="px-5 py-8 text-center text-gray-500 text-sm">{{ __('لا توجد تعيينات مسجّلة في الفترة.') }}</li>
            @endforelse
        </ul>
    </div>
</div>
