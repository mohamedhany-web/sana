...
@section('title', __('لوحة المحاسب'))
@section('header', __('لوحة المحاسب'))

@section('content')
<div class="space-y-6">
    <p class="text-sm text-gray-600">{{ __('نظرة عامة على الطلبات المالية والاتفاقيات ودفعات الرواتب. لاعتماد الطلبات أو الصرف استخدم لوحة الإدارة إن كانت لديك صلاحية.') }}</p>

    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="rounded-2xl border-2 border-amber-200/60 bg-gradient-to-br from-white to-amber-50/80 p-5 shadow-sm">
            <p class="text-sm font-semibold text-gray-600 mb-1">{{ __('طلبات دفع معلّقة') }}</p>
        ...
        <div class="rounded-2xl border-2 border-emerald-200/60 bg-gradient-to-br from-white to-emerald-50/80 p-5 shadow-sm">
            <p class="text-sm font-semibold text-gray-600 mb-1">{{ __('اتفاقيات نشطة') }}</p>
        ...
        <div class="rounded-2xl border-2 border-sky-200/60 bg-gradient-to-br from-white to-sky-50/80 p-5 shadow-sm">
            <p class="text-sm font-semibold text-gray-600 mb-1">{{ __('رواتب بانتظار الصرف') }}</p>
        ...
        <div class="rounded-2xl border-2 border-indigo-200/60 bg-gradient-to-br from-white to-indigo-50/80 p-5 shadow-sm">
            <p class="text-sm font-semibold text-gray-600 mb-1">{{ __('دفعات مصروفة هذا الشهر') }}</p>
    ...
    <div class="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
            <h2 class="text-base font-bold text-gray-900">{{ __('آخر طلبات الدفع المعلّقة') }}</h2>
            <span class="text-xs text-gray-500">{{ __('حتى 10 طلبات') }}</span>
        </div>
        <div class="overflow-x-auto">
            <table class="min-w-full text-sm">
                <thead class="bg-gray-50 text-gray-600 font-semibold">
                    <tr>
                        <th class="text-right px-4 py-3">{{ __('المستخدم') }}</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    @empty
                    <tr>
                        <td colspan="4" class="px-4 py-10 text-center text-gray-500">{{ __('لا توجد طلبات معلّقة حالياً.') }}</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
