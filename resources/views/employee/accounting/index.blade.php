...
@section('title', __('الراتب والمحاسبة'))
@section('header', __('الراتب والمحاسبة'))
...
<div class="space-y-6">
    ...
    @if($activeAgreement)
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="dashboard-card rounded-2xl p-6 card-hover-effect border-2 border-green-200/50 hover:border-green-300/70 shadow-xl hover:shadow-2xl transition-all duration-300" style="background: linear-gradient(135deg, rgba(255, 255, 255, 0.98) 0%, rgba(240, 253, 250, 0.95) 50%, rgba(209, 250, 229, 0.9) 100%);">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm font-semibold text-gray-600 mb-1">{{ __('الراتب الأساسي') }}</p>
            ...
            <div class="dashboard-card rounded-2xl p-6 card-hover-effect border-2 border-red-200/50 hover:border-red-300/70 shadow-xl hover:shadow-2xl transition-all duration-300" style="background: linear-gradient(135deg, rgba(255, 255, 255, 0.98) 0%, rgba(254, 242, 242, 0.95) 50%, rgba(254, 226, 226, 0.9) 100%);">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm font-semibold text-gray-600 mb-1">{{ __('خصومات الشهر الحالي') }}</p>
            ...
            <div class="dashboard-card rounded-2xl p-6 card-hover-effect border-2 border-blue-200/50 hover:border-blue-300/70 shadow-xl hover:shadow-2xl transition-all duration-300" style="background: linear-gradient(135deg, rgba(255, 255, 255, 0.98) 0%, rgba(240, 249, 255, 0.95) 50%, rgba(224, 242, 254, 0.9) 100%);">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm font-semibold text-gray-600 mb-1">{{ __('صافي الراتب') }}</p>
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 border border-gray-200">
            <h2 class="text-xl font-bold text-gray-900 mb-4">{{ __('معلومات الاستحقاق') }}</h2>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <p class="text-sm font-semibold text-gray-600 mb-2">{{ __('تاريخ الاستحقاق القادم') }}</p>
    ...
    @else
        <div class="bg-yellow-50 border border-yellow-200 rounded-xl p-6 text-center">
            ...
            <h3 class="text-lg font-semibold text-yellow-900 mb-2">{{ __('لا توجد اتفاقية نشطة') }}</h3>
            <p class="text-yellow-700">{{ __('لم يتم تعيين اتفاقية عمل نشطة لك حتى الآن') }}</p>
        </div>
    @endif
    ...
    <div class="bg-white rounded-xl shadow-lg p-6 border border-gray-200">
        <h2 class="text-xl font-bold text-gray-900 mb-4"><i class="fas fa-list-alt text-blue-600 mr-2"></i>{{ __('سجل المدفوعات') }}</h2>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">{{ __('إيصال التحويل') }}</th>
                    </tr>
                </thead>
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            @if($payment->status === 'paid')
                                <span class="px-2 py-1 text-xs font-semibold rounded-full bg-green-100 text-green-800">{{ __('مدفوعة') }}</span>
                                @if($payment->paid_at)<span class="block text-xs text-gray-500 mt-1">{{ $payment->paid_at->format('Y-m-d') }}</span>@endif
                            @elseif($payment->status === 'pending')
                                <span class="px-2 py-1 text-xs font-semibold rounded-full bg-yellow-100 text-yellow-800">{{ __('معلقة') }}</span>
                            @else
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            @if($payment->status === 'paid' && $payment->transfer_receipt_path)
                                <a href="{{ public_storage_url($payment->transfer_receipt_path) }}" target="_blank" class="inline-flex items-center gap-1 text-sm text-blue-600 hover:text-blue-800 font-medium"><i class="fas fa-receipt"></i> {{ __('تحميل الإيصال') }}</a>
                            @else
    ...
    @if($activeAgreement)
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 border border-gray-200">
            <h2 class="text-xl font-bold text-gray-900 mb-4">{{ __('الدفعات القادمة') }}</h2>
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 border border-gray-200">
            <h2 class="text-xl font-bold text-gray-900 mb-4">{{ __('الخصومات الأخيرة') }}</h2>
            <div class="space-y-3">
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 border border-gray-200">
            <div class="flex items-center justify-between">
                <div>
                    <h3 class="text-lg font-semibold text-gray-900">{{ __('اتفاقية العمل') }}</h3>
                    <p class="text-sm text-gray-600 mt-1">{{ __('رقم الاتفاقية:') }} {{ $activeAgreement->agreement_number }}</p>
                </div>
