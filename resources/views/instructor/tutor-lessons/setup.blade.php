...
<div class="id-tutor-page space-y-6 pb-6 w-full">
    ...
    @if($errors->any())
        <div class="ins-flash bg-rose-50 border border-rose-200 text-rose-800">
            <p class="font-bold mb-2">{{ __('تعذّر الحفظ — راجع الحقول التالية:') }}</p>
    ...
    @if($profile->isTutorActivated())
        <div class="ins-flash bg-emerald-50 border border-emerald-300 text-emerald-900">
            ...
            حسابك <strong>{{ __('مفعّل') }}</strong> — يظهر للطلاب في «اختيار معلم» عند تطابق المادة ونمط الحجز.
        </div>
    @elseif(!empty($activationBlockers))
        <div class="ins-flash bg-amber-50 border border-amber-200 text-amber-900">
            <p class="font-bold mb-1">{{ __('لإظهار حسابك للطلاب:') }}</p>
    ...
    <section class="id-hero">
        <div class="id-hero-main relative z-[1]">
            <p class="text-xs font-bold id-tag mb-1">{{ __('إعداد الحصص') }}</p>
            <h2 class="text-xl font-black text-slate-900 m-0">{{ __('ملف المعلم والجدول') }}</h2>
            <p class="text-sm text-slate-600 mt-2 mb-0">{{ __('احفظ ملفك، أضف أوقاتك الأسبوعية، وفعّل «اختيار معلم» — بعدها يظهر اسمك للطلاب تلقائياً.') }}</p>
        </div>
    </section>

    <div class="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <form method="post" action="{{ route('instructor.tutor-lessons.setup.profile') }}" class="id-panel id-form">
            <div class="id-panel-head"><h3 class="font-bold m-0">{{ __('الملف التعريفي') }}</h3></div>
            <div class="id-panel-body space-y-4">
                @csrf
                <div>
                    <label>{{ __('العنوان المختصر') }}</label>
                ...
                <div>
                    ...
                    @if($subjects->isEmpty())
                        <p class="text-sm text-rose-600">{{ __('لا توجد مواد نشطة — أضفها من لوحة الإدارة أولاً.') }}</p>
                    @else
                ...
                <div>
                    <label>{{ __('المراحل / المسارات *') }}</label>
                    @if($years->isEmpty())
                        <p class="text-sm text-rose-600">{{ __('لا توجد مراحل نشطة — أضفها من لوحة الإدارة.') }}</p>
                    @else
                ...
                <div>
                    <label>{{ __('أنماط التوافق *') }} <span class="text-xs font-normal text-slate-500">(يجب تفعيل «اختيار معلم» على الأقل)</span></label>
                ...
                <div>
                    <label>{{ __('أنواع الحصص *') }}</label>
                    <label class="id-chip"><input type="checkbox" name="session_types[]" value="one_to_one" @checked(in_array('one_to_one', old('session_types', $profile->tutor_session_types ?? ['one_to_one']), true))> {{ __('فردي') }}</label>
                    <label class="id-chip"><input type="checkbox" name="session_types[]" value="small_group" @checked(in_array('small_group', old('session_types', $profile->tutor_session_types ?? []), true))> {{ __('مجموعة') }}</label>
                </div>
                <div>
                    <label>{{ __('مدة الحصة الافتراضية (دقيقة)') }}</label>
                ...
                <button type="submit" class="id-btn-primary w-full justify-center">{{ __('حفظ الملف') }}</button>
            </div>
        </form>

        <div class="space-y-6">
            <div class="id-panel id-form">
                <div class="id-panel-head"><h3 class="font-bold m-0">{{ __('الجدول الأسبوعي') }}</h3></div>
                <div class="id-panel-body">
                    <form method="post" action="{{ route('instructor.tutor-lessons.availability.store') }}" class="grid grid-cols-1 sm:grid-cols-4 gap-3 items-end mb-4">
                        ...
                        <button type="submit" class="id-btn-ghost w-full justify-center">{{ __('إضافة') }}</button>
                    </form>
                    @forelse($availabilities as $a)
                        <div class="flex justify-between items-center text-sm py-2 border-b border-slate-100">
                            ...
                            <form method="post" action="{{ route('instructor.tutor-lessons.availability.destroy', $a) }}">
                                @csrf @method('DELETE')
                                <button type="submit" class="text-rose-600 text-xs font-bold">{{ __('حذف') }}</button>
                            </form>
                        </div>
                    @empty
                        <p class="text-sm text-slate-500">{{ __('لم تُضف فترات بعد — أضف فترة واحدة على الأقل.') }}</p>
                    @endforelse
                </div>
            </div>
            ...
            <div class="id-panel border-2 border-amber-200">
                <div class="id-panel-head bg-amber-50/80"><h3 class="font-bold text-amber-900 m-0">{{ __('الجلسة التجريبية (اختياري)') }}</h3></div>
                <div class="id-panel-body">
                    <p class="text-sm text-amber-900 mb-3">{{ __('إن كان التفعيل التلقائي معطّلاً، يمكن إتمام جلسة تجريبية ثم إنهاؤها من «حجوزاتي».') }}</p>
                    @if($trialBooking)
                        <p class="text-sm mb-3">{{ __('طلب تجريبي:') }} {{ display_datetime($trialBooking->scheduled_at) }} — {{ $trialBooking->statusLabel() }}</p>
                    @endif
                    <form method="post" action="{{ route('instructor.tutor-lessons.setup.trial') }}" class="flex flex-col sm:flex-row gap-2">
                        ...
                        <button type="submit" class="id-btn-primary">{{ __('حجز تجريبي') }}</button>
                    </form>
                </div>
            </div>
            @endif
