...
<div class="id-tutor-page space-y-6 pb-6 w-full max-w-3xl">
    ...
    <div class="id-panel">
        ...
        <div class="id-panel-body space-y-3">
            ...
            @if($evaluation)
                <div class="rounded-xl border border-emerald-200 bg-emerald-50/60 p-4 space-y-2 text-sm">
                    <p class="font-black text-emerald-900 m-0">{{ __('tutor.your_evaluation') }}</p>
                    <div class="flex flex-wrap gap-4">
                        <span>{{ __('tutor.student_rating_label') }}: <strong>{{ (int) $evaluation->rating }}★</strong></span>
                        <span>{{ __('tutor.lesson_rating_label') }}: <strong>{{ (int) ($evaluation->lesson_rating ?? $evaluation->rating) }}★</strong></span>
                    </div>
                    @if($evaluation->comment)
                        <p class="m-0 text-slate-700 whitespace-pre-line">{{ $evaluation->comment }}</p>
                    @endif
                    <p class="text-[11px] text-emerald-800 m-0">{{ __('tutor.evaluation_goes_to_parent_note') }}</p>
                </div>
            @endif
            ...
            <div class="flex flex-wrap gap-2 pt-3">
                ...
                @if($booking->status === 'completed')
                    @if(! $evaluation)
                        <div class="rounded-xl border border-amber-200 bg-amber-50 text-amber-900 text-sm p-3 font-bold w-full">
                            {{ __('tutor.evaluation_required_banner') }}
                        </div>
                        <a href="{{ route('instructor.tutor-lessons.bookings.rate', $booking) }}" class="id-btn-primary">{{ __('tutor.rate_now_required') }}</a>
                    @else
                        <a href="{{ route('instructor.tutor-lessons.bookings.rate', $booking) }}" class="id-btn-ghost">{{ __('tutor.update_evaluation') }}</a>
                    @endif
                @endif
            </div>
        </div>
