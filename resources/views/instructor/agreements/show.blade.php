    ...
    <section class="rounded-2xl bg-white backdrop-blur border-2 border-slate-200 shadow-xl overflow-hidden">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 p-5 sm:p-8">
            <div class="dashboard-card rounded-xl border-2 border-emerald-200/50 bg-gradient-to-br from-emerald-50 to-white p-5 shadow-lg">
                ...
                <p class="text-2xl font-black text-emerald-700">{{ number_format($stats['total_earned'], 2) }} {{ __('public.currency') }}</p>
            </div>
                        ...
                        <tbody class="divide-y divide-slate-200 bg-white text-sm">
                            @forelse($agreement->payments as $payment)
                                <tr class="hover:bg-slate-50 transition-colors">
                                    ...
                                    <td class="px-6 py-4 font-black text-slate-900">{{ number_format($payment->amount, 2) }} {{ __('public.currency') }}</td>
                                    <td class="px-6 py-4">
                                        <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold 
                                    ...
                                    <td class="px-6 py-4">
                                        @if($payment->status == 'paid' && $payment->transfer_receipt_path)
                                            <a href="{{ public_storage_url($payment->transfer_receipt_path) }}" target="_blank" class="inline-flex items-center gap-1 px-2.5 py-1 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg text-xs font-semibold">
