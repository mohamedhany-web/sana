...
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-slate-500 mt-1">{{ __('إنشاء وإدارة جلسات البث المباشر لطلابك') }}</p>
        </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div class="bg-white rounded-xl p-4 border border-slate-200">
            ...
            <p class="text-xs text-slate-500">{{ __('إجمالي الجلسات') }}</p>
        </div>
        <div class="bg-white rounded-xl p-4 border border-red-200">
            ...
            <p class="text-xs text-slate-500">{{ __('مباشر الآن') }}</p>
        </div>
        <div class="bg-white rounded-xl p-4 border border-blue-200">
            <p class="text-2xl font-bold text-blue-600">{{ $stats['scheduled'] }}</p>
            <p class="text-xs text-slate-500">{{ __('مجدولة') }}</p>
        </div>
        <div class="bg-white rounded-xl p-4 border border-emerald-200">
            <p class="text-2xl font-bold text-emerald-600">{{ $stats['ended'] }}</p>
            <p class="text-xs text-slate-500">{{ __('منتهية') }}</p>
        </div>
    </div>

    {{-- Filters --}}
    <div class="flex gap-2 flex-wrap">
        <a href="{{ route('instructor.live-sessions.index') }}" class="px-3 py-1.5 rounded-lg text-sm {{ !request('status') ? 'bg-slate-800 text-white font-semibold' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }} transition-colors">{{ __('الكل') }}</a>
        <a href="{{ route('instructor.live-sessions.index', ['status' => 'live']) }}" class="px-3 py-1.5 rounded-lg text-sm {{ request('status') === 'live' ? 'bg-red-600 text-white font-semibold' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }} transition-colors">{{ __('مباشر') }}</a>
        <a href="{{ route('instructor.live-sessions.index', ['status' => 'scheduled']) }}" class="px-3 py-1.5 rounded-lg text-sm {{ request('status') === 'scheduled' ? 'bg-blue-600 text-white font-semibold' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }} transition-colors">{{ __('مجدولة') }}</a>
        <a href="{{ route('instructor.live-sessions.index', ['status' => 'ended']) }}" class="px-3 py-1.5 rounded-lg text-sm {{ request('status') === 'ended' ? 'bg-slate-600 text-white font-semibold' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }} transition-colors">{{ __('منتهية') }}</a>
    </div>
    ...
    <div class="space-y-3">
        ...
        <div class="bg-white rounded-xl border {{ $session->isLive() ? 'border-red-300 ring-1 ring-red-200' : 'border-slate-200' }} p-5 flex flex-col sm:flex-row sm:items-center gap-4">
            <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2 mb-1">
                    @if($session->isLive())
                        <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-red-100 text-red-600 text-xs font-bold"><span class="w-1.5 h-1.5 bg-red-600 rounded-full animate-pulse"></span> {{ __('مباشر') }}</span>
                    @elseif($session->isScheduled())
                        <span class="px-2 py-0.5 rounded-full bg-blue-100 text-blue-600 text-xs font-medium">{{ __('مجدولة') }}</span>
                    @elseif($session->isEnded())
                        <span class="px-2 py-0.5 rounded-full bg-slate-100 text-slate-500 text-xs font-medium">{{ __('منتهية') }}</span>
                    @else
                        <span class="px-2 py-0.5 rounded-full bg-amber-100 text-amber-600 text-xs font-medium">{{ __('ملغاة') }}</span>
                    @endif
                    @if($session->course)
            ...
            <div class="flex items-center gap-2 flex-shrink-0">
                @if($session->isLive())
                    ...
                    <form method="POST" action="{{ route('instructor.live-sessions.end', $session) }}" onsubmit="return confirm(@json(__('إنهاء الجلسة؟')))">
                        @csrf
                ...
                @endif
                <a href="{{ route('instructor.live-sessions.show', $session) }}" class="px-3 py-2 bg-slate-100 text-slate-600 rounded-lg text-sm hover:bg-slate-200 transition-colors" title="{{ __('تفاصيل') }}"><i class="fas fa-eye"></i></a>
            </div>
        </div>
        @empty
        <div class="text-center py-16 bg-white rounded-xl border border-slate-200">
            ...
            <p class="text-lg font-semibold text-slate-600 mb-2">{{ __('لا توجد جلسات بث بعد') }}</p>
            <p class="text-sm text-slate-500 mb-4">{{ __('أنشئ أول جلسة بث مباشر لطلابك') }}</p>
