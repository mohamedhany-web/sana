...
<div class="space-y-6">
                        ...
                        <div class="flex flex-wrap items-center gap-2 mt-1.5">
                            <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold bg-white/15 border border-white/20">
                                @if($lecture->status == 'scheduled')
                                    <i class="fas fa-calendar-alt"></i> {{ __('مجدولة
                                @elseif($lecture->status == \'in_progress\')') }}
                                    <i class="fas fa-clock"></i> {{ __('قيد التنفيذ
            ...
            <div class="flex flex-wrap items-center gap-2 shrink-0">
                <a href="{{ route('instructor.lectures.edit', $lecture) }}"
                   class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white text-indigo-700 hover:bg-white/90 border border-white/20 font-extrabold transition-colors">
                    ...
                    <span>{{ __('common.edit') ?? __('تعديل') }}</span>
                </a>
                <a href="{{ route('instructor.lectures.index') }}"
                   class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white/15 hover:bg-white/20 border border-white/20 text-white font-semibold transition-colors">
                    ...
                    <span>{{ __('common.back') ?? __('العودة') }}</span>
                </a>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-3 sm:gap-4">
        <div class="stats-mini-lecture rounded-2xl p-4 text-center shadow-sm">
            ...
            <div class="text-xs text-slate-600 font-semibold mt-1">{{ __('حاضر') }}</div>
        </div>
        <div class="stats-mini-lecture rounded-2xl p-4 text-center shadow-sm">
            ...
            <div class="text-xs text-slate-600 font-semibold mt-1">{{ __('متأخر') }}</div>
        </div>
        <div class="stats-mini-lecture rounded-2xl p-4 text-center shadow-sm">
            ...
            <div class="text-xs text-slate-600 font-semibold mt-1">{{ __('جزئي') }}</div>
        </div>
        <div class="stats-mini-lecture rounded-2xl p-4 text-center shadow-sm">
            ...
            <div class="text-xs text-slate-600 font-semibold mt-1">{{ __('غائب') }}</div>
        </div>
        <div class="stats-mini-lecture rounded-2xl p-4 text-center shadow-sm">
            ...
            <div class="text-xs text-slate-600 font-semibold mt-1">{{ __('إجمالي') }}</div>
        </div>
    </div>
            ...
            <div class="dashboard-card-lecture rounded-2xl p-5 sm:p-6 shadow-lg">
                ...
                <div class="space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-bold text-slate-500 mb-2 uppercase tracking-wide">{{ __('الحالة') }}</label>
                            <div class="flex items-center gap-2">
                                <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold shadow-md
                                    ...
                                    @endif">
                                    @if($lecture->status == 'scheduled') {{ __('مجدولة') }}
                                    @elseif($lecture->status == 'in_progress') {{ __('قيد التنفيذ') }}
                                    @elseif($lecture->status == 'completed') {{ __('مكتملة') }}
                                    @else {{ __('ملغاة') }}
                                    @endif
                                </span>
                                ...
                                <select onchange="updateStatus(this.value)"
                                    ...
                                    <option value="scheduled" {{ $lecture->status == 'scheduled' ? 'selected' : '' }}>{{ __('مجدولة') }}</option>
                                    <option value="in_progress" {{ $lecture->status == 'in_progress' ? 'selected' : '' }}>{{ __('قيد التنفيذ') }}</option>
                                    <option value="completed" {{ $lecture->status == 'completed' ? 'selected' : '' }}>{{ __('مكتملة') }}</option>
                                    <option value="cancelled" {{ $lecture->status == 'cancelled' ? 'selected' : '' }}>{{ __('ملغاة') }}</option>
                                </select>
                                @endif
                            </div>
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-slate-500 mb-2 uppercase tracking-wide">{{ __('التاريخ والوقت') }}</label>
                        ...
                        <div>
                            <label class="block text-xs font-bold text-slate-500 mb-2 uppercase tracking-wide">{{ __('المدة') }}</label>
                            <div class="flex items-center gap-2 text-slate-800 font-black">
                        ...
                        <div>
                            <label class="block text-xs font-bold text-slate-500 mb-2 uppercase tracking-wide">{{ __('الكورس') }}</label>
                            <div class="flex items-center gap-2 text-slate-800 font-bold">
                                <i class="fas fa-book text-sky-600"></i>
                                {{ $lecture->course->title ?? __('غير محدد') }}
                            </div>
                        </div>
                        @if($lecture->lesson)
                        <div>
                            <label class="block text-xs font-bold text-slate-500 mb-2 uppercase tracking-wide">{{ __('الدرس') }}</label>
                            <div class="flex items-center gap-2 text-slate-800 font-bold">
                    ...
                    <div class="pt-4 border-t-2 border-slate-200">
                        <label class="block text-xs font-bold text-slate-500 mb-2 uppercase tracking-wide">{{ __('الوصف') }}</label>
                    ...
                    @if($lecture->notes)
                    <div class="pt-4 border-t-2 border-slate-200">
                        <label class="block text-xs font-bold text-slate-500 mb-2 uppercase tracking-wide">{{ __('الملاحظات') }}</label>
                        <div class="text-slate-700 font-medium bg-slate-50 p-4 rounded-xl border border-slate-200">
                            {{ $lecture->notes }}
                        </div>
                    </div>
                    ...
                    <div class="pt-4 border-t-2 border-slate-200">
                        <label class="block text-xs font-bold text-slate-500 mb-3 uppercase tracking-wide">{{ __('الروابط') }}</label>
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                            <a href="{{ $lecture->recording_url }}" target="_blank"
                               class="flex items-center gap-3 p-4 bg-violet-50 rounded-xl border-2 border-violet-200 hover:bg-violet-100 hover:border-violet-300 transition-all shadow-sm">
                                ...
                                <div class="flex-1">
                                    <div class="font-black text-slate-800 text-sm">{{ __('تسجيل المحاضرة') }}</div>
                                    <div class="text-xs text-slate-600 font-medium mt-0.5">{{ __('رابط التسجيل') }}</div>
                                </div>
            ...
            <div class="dashboard-card-lecture rounded-2xl p-5 sm:p-6 shadow-lg">
                <div class="flex items-center justify-between mb-4 pb-4 border-b-2 border-slate-200">
                    <h3 class="text-lg sm:text-xl font-black text-slate-800 flex items-center gap-2">
                        ...
                        {{ __('إدارة الحضور والغياب') }}
                    </h3>
                    <div class="text-sm text-slate-600 font-bold">
                        {{ __('إجمالي:') }} <span class="text-sky-600">{{ $attendanceStats['total_students'] }}</span>
                    </div>
                </div>
                <div>
                        ...
                        <div class="overflow-x-auto rounded-xl border border-slate-200">
                            <table class="min-w-full divide-y divide-slate-200">
                                <thead class="bg-slate-50">
                                    <tr>
                                        <th class="px-4 py-3 text-right text-xs font-bold text-slate-700 uppercase tracking-wider">{{ __('الطالب') }}</th>
                                        <th class="px-4 py-3 text-right text-xs font-bold text-slate-700 uppercase tracking-wider">{{ __('الحالة') }}</th>
                                        <th class="px-4 py-3 text-right text-xs font-bold text-slate-700 uppercase tracking-wider">{{ __('دقائق الحضور') }}</th>
                                        <th class="px-4 py-3 text-right text-xs font-bold text-slate-700 uppercase tracking-wider">{{ __('الإجراءات') }}</th>
                                    </tr>
                                </thead>
                                    ...
                                    <tr class="student-row-lecture">
                                            ...
                                            <div class="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <div class="text-sm font-black text-slate-800">
                                                        {{ $enrollment->user->name ?? __('غير محدد') }}
                                                    </div>
                                        ...
                                        <td class="px-4 py-4 whitespace-nowrap">
                                            <select onchange="updateAttendance({{ $enrollment->user_id }}, this.value)"
                                                ...
                                                <option value="present" {{ $record && $record->status == 'present' ? 'selected' : '' }}>{{ __('حاضر') }}</option>
                                                <option value="late" {{ $record && $record->status == 'late' ? 'selected' : '' }}>{{ __('متأخر') }}</option>
                                                <option value="partial" {{ $record && $record->status == 'partial' ? 'selected' : '' }}>{{ __('جزئي') }}</option>
                                                <option value="absent" {{ !$record || $record->status == 'absent' ? 'selected' : '' }}>{{ __('غائب') }}</option>
                                            </select>
                                        </td>
                                    </tr>
                                    @endforeach
                    ...
                    @else
                        <div class="text-center py-12">
                            ...
                            <p class="text-lg font-black text-slate-800 mb-2">{{ __('لا يوجد طلاب مسجلين') }}</p>
                            <p class="text-sm text-slate-500 font-medium">{{ __('لا يوجد طلاب مسجلين في هذا الكورس') }}</p>
                        </div>
                    @endif
                </div>
            </div>
            ...
            <div class="dashboard-card-lecture rounded-2xl p-5 shadow-lg">
                ...
                <div class="space-y-3">
                    <div class="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-200">
                        <span class="text-sm text-slate-700 font-bold">{{ __('تتبع الحضور') }}</span>
                        <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold shadow-md {{ $lecture->has_attendance_tracking ? 'bg-emerald-600 text-white' : 'bg-slate-400 text-white' }}">
                            ...
                            {{ $lecture->has_attendance_tracking ? __('مفعل') : __('معطل') }}
                        </span>
                    </div>
                    <div class="flex items-center justify-between p-3 bg-amber-50 rounded-xl border border-amber-200">
                        <span class="text-sm text-slate-700 font-bold">{{ __('يوجد واجب') }}</span>
                        <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold shadow-md {{ $lecture->has_assignment ? 'bg-emerald-600 text-white' : 'bg-slate-400 text-white' }}">
                            ...
                            {{ $lecture->has_assignment ? __('نعم') : __('لا') }}
                        </span>
                    </div>
                    <div class="flex items-center justify-between p-3 bg-violet-50 rounded-xl border border-violet-200">
                        <span class="text-sm text-slate-700 font-bold">{{ __('يوجد تقييم') }}</span>
                        <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold shadow-md {{ $lecture->has_evaluation ? 'bg-emerald-600 text-white' : 'bg-slate-400 text-white' }}">
                            ...
                            {{ $lecture->has_evaluation ? __('نعم') : __('لا') }}
                        </span>
                    </div>
                </div>
            </div>
                ...
                <div class="space-y-2">
                    <a href="{{ route('instructor.lectures.edit', $lecture) }}"
                       class="flex items-center gap-3 p-3 bg-sky-50 hover:bg-sky-100 rounded-xl border border-sky-200 transition-all">
                        ...
                        <span class="font-bold text-slate-800 text-sm">{{ __('تعديل المحاضرة') }}</span>
                    </a>
                    @if($lecture->course)
                    <a href="{{ route('instructor.courses.show', $lecture->course) }}"
                       class="flex items-center gap-3 p-3 bg-emerald-50 hover:bg-emerald-100 rounded-xl border border-emerald-200 transition-all">
                        ...
                        <span class="font-bold text-slate-800 text-sm">{{ __('عرض الكورس') }}</span>
                    </a>
                    @endif
                    <a href="{{ route('instructor.attendance.lecture', $lecture) }}"
                       class="flex items-center gap-3 p-3 bg-blue-50 hover:bg-blue-100 rounded-xl border border-blue-200 transition-all">
                        ...
                        <span class="font-bold text-slate-800 text-sm">{{ __('تفاصيل الحضور') }}</span>
                    </a>
                </div>
            </div>
        </div>
