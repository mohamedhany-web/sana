...
<div class="space-y-6 w-full max-w-full" x-data="personalBrandingForm(@js(old('skills', $profile->skills)), @js($skillsPreview))">
    ...
    {{-- شريط الحالة --}}
    <div class="rounded-2xl bg-white border border-slate-200 shadow-sm p-4 sm:p-5 ring-2 {{ $statusStyles['ring'] }}">
        <div class="flex flex-col sm:flex-row sm:items-center gap-3 sm:justify-between">
            <div class="flex items-start gap-3 min-w-0">
                ...
                <div class="min-w-0">
                    <p class="text-xs font-bold text-slate-500 uppercase tracking-wide m-0 mb-1">{{ __('instructor.status_label') }}</p>
            ...
            <p class="text-xs text-slate-500 m-0 sm:text-left max-w-md">{{ __('instructor.personal_branding_desc') }}</p>
        </div>
    </div>
        ...
        <div class="lg:col-span-8">
            <form method="POST" action="{{ route('instructor.personal-branding.update') }}" enctype="multipart/form-data"
                ...
                <div class="p-5 sm:p-6 border-b border-slate-200">
                    ...
                    <div class="mt-4 flex flex-col sm:flex-row gap-5 items-center">
                        ...
                        <label class="flex-1 w-full cursor-pointer rounded-2xl border-2 border-dashed border-slate-300 hover:border-indigo-400 bg-slate-50 hover:bg-indigo-50/40 p-5 text-center transition-colors">
                            ...
                            <p class="text-sm font-semibold text-slate-700 m-0">{{ __('instructor.choose_image_label') }}</p>
                ...
                <div class="p-5 sm:p-6 border-b border-slate-200 space-y-4">
                    ...
                    <div>
                        <label for="pb-headline" class="block text-sm font-semibold text-slate-700 mb-2">{{ __('instructor.intro_title') }}</label>
                    ...
                    <div>
                        <label for="pb-bio" class="block text-sm font-semibold text-slate-700 mb-2">{{ __('instructor.bio') }}</label>
                        <textarea id="pb-bio" name="bio" rows="5" placeholder="{{ __('instructor.bio_placeholder') }}"
                                  class="w-full px-4 py-2.5 border border-slate-200 rounded-xl bg-slate-50 text-slate-800 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 text-sm">{{ old('bio', $profile->bio) }}</textarea>
                ...
                <div class="p-5 sm:p-6">
                    ...
                    <p class="text-xs text-slate-500 mt-2 mb-3 m-0">{{ __('instructor.skills_hint') }}</p>
            ...
            <form method="POST" action="{{ route('instructor.personal-branding.submit') }}" class="mt-4">
                ...
                <div class="rounded-2xl border border-amber-200 bg-amber-50/80 p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    ...
                    <button type="submit" @disabled(!$canSubmit)
                            ...
                            {{ $canSubmit ? 'bg-amber-500 hover:bg-amber-600 text-white cursor-pointer' : 'bg-slate-300 text-slate-500 cursor-not-allowed' }}">
        ...
        <aside class="lg:col-span-4 space-y-4">
            ...
            <div class="rounded-2xl bg-white border border-slate-200 shadow-sm overflow-hidden">
                ...
                <div class="p-4">
                    <div class="flex items-center gap-3 mb-3">
                        ...
                        <div class="min-w-0">
                            <p class="text-sm font-bold text-slate-900 m-0 truncate">{{ $user->name }}</p>
            ...
            <div class="rounded-2xl bg-white border border-slate-200 shadow-sm p-5">
                ...
                <ul class="space-y-2.5 m-0 p-0 list-none text-sm">
                    ...
                    <li class="flex items-center gap-2 {{ $check['ok'] ? 'text-emerald-700' : 'text-slate-500' }}">
