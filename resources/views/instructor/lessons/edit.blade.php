...
@section('title', __('تعديل: ') . $lesson->title)
...
<div class="space-y-6">
    ...
    <div class="rounded-2xl p-5 sm:p-6 bg-white border border-slate-200 shadow-sm">
        <nav class="flex items-center gap-2 text-sm text-slate-500 mb-2 flex-wrap">
            ...
            <span>/</span>
            <span class="text-slate-700 font-medium">{{ __('تعديل') }}</span>
        </nav>
        <h1 class="text-2xl font-bold text-slate-800">{{ __('تعديل الدرس') }}</h1>
    </div>

    @if ($errors->any())
    <div class="rounded-xl p-4 bg-red-50 border border-red-200 text-red-800">
        <p class="font-semibold mb-2"><i class="fas fa-exclamation-circle ml-2"></i> {{ __('يرجى تصحيح الأخطاء:') }}</p>
            ...
            <div class="space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="md:col-span-2">
                        <label class="block text-sm font-semibold text-slate-700 mb-2">{{ __('عنوان الدرس') }} <span class="text-red-500">*</span></label>
                        <input type="text" name="title" value="{{ old('title', $lesson->title) }}" required
                               ...
                               placeholder="{{ __('عنوان الدرس') }}">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-2">{{ __('نوع الدرس') }} <span class="text-red-500">*</span></label>
                        <select name="type" id="lessonType" required
                                class="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:border-sky-500 focus:ring-2 focus:ring-sky-500/20 text-slate-800">
                            <option value="video" {{ old('type', $lesson->type) == 'video' ? 'selected' : '' }}>{{ __('فيديو') }}</option>
                            <option value="text" {{ old('type', $lesson->type) == 'text' ? 'selected' : '' }}>{{ __('نص') }}</option>
                            <option value="document" {{ old('type', $lesson->type) == 'document' ? 'selected' : '' }}>{{ __('ملف') }}</option>
                            <option value="quiz" {{ old('type', $lesson->type) == 'quiz' ? 'selected' : '' }}>{{ __('اختبار') }}</option>
                        </select>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea name="description" rows="3" class="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:border-sky-500 focus:ring-2 focus:ring-sky-500/20 text-slate-800"
                              placeholder="{{ __('وصف مختصر') }}">{{ old('description', $lesson->description) }}</textarea>
                </div>

                <div class="video-section {{ old('type', $lesson->type) != 'video' ? 'hidden' : '' }} rounded-xl p-4 bg-slate-50 border border-slate-200">
                    <h4 class="font-semibold text-slate-800 mb-3 flex items-center gap-2"><i class="fas fa-video text-red-500"></i> {{ __('إعدادات الفيديو') }}</h4>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-slate-600 mb-2">{{ __('رابط الفيديو') }}</label>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-slate-600 mb-2">{{ __('أو رفع ملف فيديو (حتى 500MB)') }}</label>
                ...
                <div class="text-section {{ old('type', $lesson->type) != 'text' ? 'hidden' : '' }}">
                    <label class="block text-sm font-semibold text-slate-700 mb-2">{{ __('محتوى الدرس') }}</label>
                    <textarea name="content" rows="8" class="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:border-sky-500 focus:ring-2 focus:ring-sky-500/20 text-slate-800"
                              placeholder="{{ __('محتوى الدرس...') }}">{{ old('content', $lesson->content) }}</textarea>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-2">{{ __('مدة الدرس (دقيقة)') }}</label>
                        <input type="number" name="duration_minutes" value="{{ old('duration_minutes', $lesson->duration_minutes) }}" min="0"
                               class="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:border-sky-500 text-slate-800">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-2">{{ __('الترتيب') }} <span class="text-red-500">*</span></label>
                ...
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">{{ __('مرفقات إضافية') }}</label>
                    <input type="file" name="attachments[]" multiple
                           class="w-full px-4 py-2.5 border border-slate-200 rounded-xl text-slate-800">
                    <p class="text-xs text-slate-500 mt-1">{{ __('حتى 40 ميجابايت لكل ملف. المرفقات الحالية تبقى مضافاً عليها الجديد.') }}</p>
                </div>

                <div class="rounded-xl p-4 bg-slate-50 border border-slate-200 space-y-3">
                    <h4 class="font-semibold text-slate-800">{{ __('خيارات الدرس') }}</h4>
                    <label class="flex items-center gap-3 cursor-pointer">
                        ...
                        <span class="text-sm font-medium text-slate-700">{{ __('الدرس نشط') }}</span>
                    </label>
                    <label class="flex items-center gap-3 cursor-pointer">
                        <input type="checkbox" name="is_free" value="1" {{ old('is_free', $lesson->is_free) ? 'checked' : '' }}
                               class="w-4 h-4 rounded border-slate-300 text-sky-500 focus:ring-sky-500">
                        <span class="text-sm font-medium text-slate-700">{{ __('درس مجاني (معاينة)') }}</span>
                    </label>
                </div>
            </div>
