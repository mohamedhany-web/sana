...
<section class="sana-section" id="categories">
    <div class="sana-container">
        ...
        <div class="sana-cats-row sana-reveal">
            @foreach($cats as $cat)
            <a href="{{ $cat['url'] ?? route('public.courses') }}" class="sana-cat-m" style="background:{{ $cat['bg'] }}">
                <span class="sana-cat-m__emoji">{{ $cat['emoji'] }}</span>
                <span class="sana-cat-m__name">{{ $cat['name'] }}</span>
            </a>
            @endforeach
        </div>
    </div>
