...
@endphp
<article class="{{ $widthClass }} {{ $fluid ? '' : 'shrink-0' }} {{ $cardClass }} p-6 sm:p-8 flex flex-col h-full dark:bg-slate-800 dark:border-slate-700">
    ...
    <div class="flex flex-col flex-1">
        ...
        <div class="mt-5 pt-4 border-t border-slate-100 dark:border-slate-600">
            @if($t->author_name)
                <p class="font-bold text-sm text-slate-900 dark:text-white">{{ $t->author_name }}</p>
            @endif
            @if($t->role_label)
                <p class="text-xs mt-1 text-slate-500 dark:text-slate-400">{{ $t->role_label }}</p>
            @endif
