...
<style>
    ...
    .tl-page { --tl-blue: {{ $b['blue'] }}; --tl-purple: {{ $b['purple'] }}; --tl-gold: {{ $b['yellow'] }};
        --tl-grad: linear-gradient(135deg, {{ $b['blue'] }} 0%, {{ $b['purple'] }} 55%, {{ $b['purple_dark'] }} 100%); }
    .tl-hero {
        border-radius: 22px;
        ...
        position: relative;
        overflow: hidden;
        box-shadow: 0 20px 50px -24px rgba({{ $b['blue_rgb'] }}, .45);
    }
    .tl-hero::after {
        content: '';
        position: absolute;
        ...
        height: 200px;
        border-radius: 50%;
        background: radial-gradient(circle, rgba({{ $b['yellow_rgb'] }}, .35), transparent 70%);
        top: -60px;
        left: -40px;
        pointer-events: none;
    }
    ...
    .tl-card {
        ...
        border-radius: 18px;
        padding: 1.25rem;
        box-shadow: 0 10px 32px -20px rgba({{ $b['purple_rgb'] }}, .18);
    }
    .tl-stat {
        border-radius: 16px;
        padding: 1rem 1.1rem;
        background: linear-gradient(135deg, rgba({{ $b['blue_rgb'] }}, .06), rgba({{ $b['purple_rgb'] }}, .04));
        border: 1px solid rgba({{ $b['blue_rgb'] }}, .12);
    }
    ...
    .tl-btn-primary { background: var(--tl-grad); color: #fff; box-shadow: 0 8px 24px -10px rgba({{ $b['purple_rgb'] }}, .5); }
    ...
    .tl-form input:focus, .tl-form select:focus, .tl-form textarea:focus {
        ...
        box-shadow: 0 0 0 3px rgba({{ $b['blue_rgb'] }}, .12);
    }
    .tl-chip {
        display: inline-flex;
        align-items: center;
    ...
    .tl-chip:has(input:checked) { border-color: var(--tl-blue); background: rgba({{ $b['blue_rgb'] }}, .08); color: var(--tl-blue); font-weight: 700; }
