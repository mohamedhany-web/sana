...
<header class="stu-topbar flex items-center justify-between gap-2 sm:gap-3 px-3 sm:px-4 md:px-6 flex-shrink-0 sticky top-0 z-30">
    <div class="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
        ...
        <div class="min-w-0 flex-1">
            <p class="text-xs sm:text-sm font-bold text-teal-800 truncate sm:hidden">{{ __('parent.portal_title') }}</p>
            <div class="hidden sm:block max-w-md min-w-0">
                <p class="text-sm font-bold text-teal-800 truncate">{{ __('parent.portal_title') }}</p>
                <p class="text-xs text-slate-500 truncate hidden md:block">{{ __('parent.dashboard_subtitle') }}</p>
            </div>
        </div>
    </div>
        ...
        <div class="relative" x-data="{ open: false }">
            ...
            <div x-show="open" @click.away="open = false" x-cloak x-transition
                ...
                <div class="px-4 py-3 border-b border-slate-100 flex items-center justify-between gap-2">
                    ...
                    <a href="{{ route('notifications') }}" class="text-xs font-bold text-teal-700 hover:underline">
                        {{ __('student.view_all') }}
                    </a>
                </div>
                @if($navRecentNotifications->isNotEmpty())
                    <div class="max-h-96 overflow-y-auto">
                        @foreach($navRecentNotifications as $notification)
                            ...
                            @endphp
                            <a href="{{ $notificationUrl }}" class="block px-4 py-3 border-b border-slate-50 hover:bg-teal-50/50 transition-colors no-underline text-inherit">
        ...
        <div class="relative" x-data="{ open: false }">
            ...
            <div x-show="open" @click.away="open = false" x-cloak x-transition
                ...
                <div class="p-1.5 space-y-0.5">
                    <a href="{{ route('parent.profile') }}" class="flex items-center gap-2.5 px-3 py-2 rounded-xl text-sm font-semibold text-slate-700 hover:bg-slate-50 no-underline">
                        <i class="fas fa-user text-slate-400 w-4 text-center text-xs"></i>
                        {{ __('parent.profile') }}
                    </a>
                    <a href="{{ route('parent.settings') }}" class="flex items-center gap-2.5 px-3 py-2 rounded-xl text-sm font-semibold text-slate-700 hover:bg-slate-50 no-underline">
