...
<header class="stu-topbar flex items-center justify-between gap-3 px-4 md:px-6 flex-shrink-0 sticky top-0 z-30">
    <div class="flex items-center gap-3 flex-1 min-w-0">
        ...
        <div class="hidden sm:block flex-1 max-w-md">
            <p class="text-sm font-bold text-slate-700 dark:text-slate-200 truncate">{{ __('student.dashboard_title') }}</p>
        </div>
        @endhasPermission
    </div>
        ...
        <div class="relative" x-data="{ open: false }">
            ...
            <div x-show="open" @click.away="open = false" x-cloak x-transition
                ...
                <div class="px-4 py-3 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between gap-2">
                    <h3 class="text-sm font-bold text-slate-900 dark:text-slate-100">{{ __('student.notifications') }}</h3>
        ...
        <div class="relative" x-data="{ open: false }">
            ...
            <div x-show="open" @click.away="open = false" x-cloak x-transition
                ...
                <div class="p-1.5 space-y-0.5">
                    <a href="{{ route('profile') }}" class="flex items-center gap-2.5 px-3 py-2 rounded-xl text-sm font-semibold text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800 no-underline">
