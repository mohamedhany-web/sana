            ...
            <div class="flex flex-col xl:flex-row xl:items-center justify-between gap-6">
                <div class="flex flex-col sm:flex-row sm:items-center gap-5">
                    <div class="profile-avatar mx-auto sm:mx-0">
                        @if($user->profile_image)
                            <img src="{{ $user->profile_image_url }}" alt="" onerror="this.style.display='none'; this.nextElementSibling?.classList.remove('hidden');">
                            <span class="hidden">{{ mb_substr($displayName, 0, 1) }}</span>
                        @else
                    ...
                    <div class="text-center sm:text-start min-w-0">
                        ...
                        <h1 class="hero-title text-2xl sm:text-3xl font-heading font-bold mb-1">{{ $displayName }}</h1>
                        <p class="hero-sub text-sm mb-3">إدارة بياناتك وإعدادات حسابك على {{ $platformName ?? config('brand.name', config('app.name')) }}</p>
                        <div class="flex flex-wrap justify-center sm:justify-start gap-2">
                            @if($user->phone)
                                <span class="profile-contact-pill"><i class="fas fa-phone"></i>{{ $user->phone }}</span>
                            @endif
                            @if($user->email)
                                <span class="profile-contact-pill"><i class="fas fa-envelope"></i>{{ $user->email }}</span>
                            @endif
                        </div>
                    </div>
                </div>
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-2 xl:max-w-md xl:w-full shrink-0">
                    <div class="profile-hero-stat">
                        ...
                        <div class="profile-hero-stat__value">{{ $memberSince }}</div>
                    </div>
                    <div class="profile-hero-stat">
                        <div class="profile-hero-stat__label">نوع الحساب</div>
                        <div class="profile-hero-stat__value">{{ $roleLabel }}</div>
                    </div>
                    <div class="profile-hero-stat">
                        <div class="profile-hero-stat__label">آخر تسجيل دخول</div>
                        <div class="profile-hero-stat__value">{{ $lastLogin }}</div>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="admin-side-card">
                ...
                <div class="admin-side-card__body">
                    <div class="admin-info-row">
                        ...
                        <span class="admin-info-row__value">#{{ str_pad($user->id, 5, '0', STR_PAD_LEFT) }}</span>
                    </div>
                    <div class="admin-info-row">
                        <span class="admin-info-row__label">نوع الحساب</span>
                        <span class="admin-chip admin-chip--primary">{{ $roleLabel }}</span>
                    </div>
                    <div class="admin-info-row {{ $user->is_active ? 'admin-info-row--success' : 'admin-info-row--danger' }}">
            ...
            <div class="admin-side-card">
                ...
                <div class="admin-side-card__body space-y-3">
                    ...
                    @else
                        ...
                        <a href="{{ route('two-factor.setup') }}" class="admin-btn admin-btn--success">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            ...
                            <div class="admin-field">
                                ...
                                <div class="admin-field-input-wrap">
                                    ...
                                    <input type="text" name="phone" value="{{ old('phone', $user->phone) }}" required class="admin-input">
                                </div>
                        ...
                        <div class="admin-field">
                            ...
                            <div class="flex flex-col sm:flex-row sm:items-center gap-4">
                                <div class="admin-upload-preview">
                                    @if($user->profile_image)
                                        <img src="{{ $user->profile_image_url }}" alt="" class="w-full h-full object-cover" onerror="this.style.display='none'; this.nextElementSibling?.classList.remove('hidden');">
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    @error('profile_image')<p class="text-rose-600 text-xs mt-1.5 font-medium">{{ $message }}</p>@enderror
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="admin-password-block space-y-4">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div class="admin-field">
                                    ...
                                    @error('current_password')<p class="text-rose-600 text-xs mt-1.5 font-medium">{{ $message }}</p>@enderror
                                </div>
                                <div class="admin-field">
