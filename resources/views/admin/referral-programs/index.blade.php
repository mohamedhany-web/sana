...
<div class="space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-4">
        ...
        <div class="flex flex-wrap items-center gap-2">
            <a href="{{ route('admin.referrals.index') }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl border border-slate-200 text-slate-700 text-sm font-semibold hover:bg-slate-50">
    ...
    <div class="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50 transition-colors">
                        <td class="px-4 py-3">
                            <a href="{{ route('admin.referral-programs.show', $program) }}" class="font-semibold text-slate-900 hover:text-emerald-600">{{ $program->name }}</a>
                            @if($program->description)
                            <p class="text-xs text-slate-500 mt-0.5">{{ \Illuminate\Support\Str::limit($program->description, 48) }}</p>
                            @endif
                        </td>
                        <td class="px-4 py-3">
                        ...
                        <td class="px-4 py-3 font-mono text-slate-700">{{ number_format($program->referrals_count ?? 0) }}</td>
                        ...
                        <td class="px-4 py-3">
                            <span class="inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium {{ ($program->is_active && $program->isValid()) ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600' }}">
        ...
        <div class="px-4 py-3 border-t border-slate-100">{{ $programs->links() }}</div>
        @endif
    </div>
    @else
