...
@section('title', __('تعديل برنامج إحالة'))
@section('header', '')

@section('content')
<div class="space-y-6">
    <div class="flex flex-wrap items-center gap-3">
        <a href="{{ route('admin.referral-programs.index') }}" class="inline-flex items-center justify-center w-9 h-9 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-600"><i class="fas fa-arrow-right"></i></a>
        <div>
            <h1 class="text-2xl font-bold text-slate-900 font-heading"><i class="fas fa-gift text-emerald-600 ml-2"></i>تعديل: {{ $referralProgram->name }}</h1>
            ...
            <form action="{{ route('admin.referral-programs.update', $referralProgram) }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3"
                        class="w-full px-4 py-3 border-2 border-gray-300 bg-white text-gray-900 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all">{{ old('description', $referralProgram->description) }}</textarea>
                </div>
                ...
                <div class="flex justify-end gap-4 pt-6 border-t border-gray-200">
                    <a href="{{ route('admin.referral-programs.index') }}" class="inline-flex items-center gap-2 px-6 py-2.5 rounded-xl border border-slate-200 text-slate-700 text-sm font-semibold hover:bg-slate-50">
                        إلغاء
                    </a>
