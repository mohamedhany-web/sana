...
<div class="space-y-6">
    <div class="flex flex-wrap items-center gap-3">
        ...
        <div>
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white font-heading"><i class="fas fa-gift text-emerald-600 ml-2"></i>تعديل: {{ $referralProgram->name }}</h1>
            ...
            <form action="{{ route('admin.referral-programs.update', $referralProgram) }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3"
                        class="w-full px-4 py-3 border-2 border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-gray-900 dark:text-slate-100 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all">{{ old('description', $referralProgram->description) }}</textarea>
                </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="number" name="referral_code_valid_days" id="referral_code_valid_days" min="1" value="{{ old('referral_code_valid_days', $referralProgram->referral_code_valid_days) }}"
                                class="w-full px-4 py-3 border-2 border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-gray-900 dark:text-slate-100 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all">
                            <p class="text-xs text-gray-500 dark:text-slate-400 mt-1">اختياري — للتوثيق؛ التسجيل يعتمد على كود المحيل</p>
                        </div>
                ...
                <div class="rounded-xl border border-violet-200 dark:border-violet-800 bg-violet-50/50 dark:bg-violet-900/20 p-4 space-y-3">
                    <div class="flex items-center gap-3">
                        ...
                        <label for="is_default" class="text-sm font-medium text-gray-800 dark:text-violet-200">برنامج <strong>افتراضي</strong> لإحالات التسجيل الجديدة</label>
                    </div>
