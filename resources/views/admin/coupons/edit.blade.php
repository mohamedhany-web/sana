...
<div class="space-y-6">
    ...
    <form action="{{ route('admin.coupons.update', $coupon) }}" method="POST" class="bg-white rounded-xl border border-slate-200 p-6 space-y-6 shadow-sm">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
                ...
                <input type="text" name="code" required value="{{ old('code', $coupon->code) }}" class="w-full rounded-lg border-slate-300 uppercase font-mono">
                @error('code')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
            </div>
            <div>
                <label class="block text-sm font-semibold text-slate-700 mb-1">العنوان <span class="text-red-500">*</span></label>
                <input type="text" name="title" required value="{{ old('title', $coupon->title ?? $coupon->name) }}" class="w-full rounded-lg border-slate-300">
            ...
            <div>
                ...
                <input type="number" name="discount_value" step="0.01" min="0" required value="{{ old('discount_value', $coupon->discount_value) }}" class="w-full rounded-lg border-slate-300">
                @error('discount_value')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
            </div>
            <div>
                <label class="block text-sm font-semibold text-slate-700 mb-1">الحد الأدنى للطلب ({{ __('public.currency') }})</label>
                <input type="number" name="minimum_amount" step="0.01" min="0" value="{{ old('minimum_amount', $coupon->minimum_amount) }}" class="w-full rounded-lg border-slate-300">
                @error('minimum_amount')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
            </div>
            <div>
                <label class="block text-sm font-semibold text-slate-700 mb-1">الحد الأقصى للخصم ({{ __('public.currency') }})</label>
                <input type="number" name="maximum_discount" step="0.01" min="0" value="{{ old('maximum_discount', $coupon->maximum_discount) }}" class="w-full rounded-lg border-slate-300">
            ...
            <div>
                ...
                <input type="number" name="usage_limit_per_user" min="1" value="{{ old('usage_limit_per_user', $coupon->usage_limit_per_user ?? 1) }}" class="w-full rounded-lg border-slate-300">
            ...
            <div>
                ...
                <input type="date" name="valid_from" value="{{ old('valid_from', $coupon->starts_at ? $coupon->starts_at->format('Y-m-d') : '') }}" class="w-full rounded-lg border-slate-300">
            ...
            <div>
                ...
                <input type="date" name="valid_until" value="{{ old('valid_until', $coupon->expires_at ? $coupon->expires_at->format('Y-m-d') : '') }}" class="w-full rounded-lg border-slate-300">
        ...
        <div>
            ...
            <textarea name="description" rows="3" class="w-full rounded-lg border-slate-300">{{ old('description', $coupon->description) }}</textarea>
        ...
        <div class="rounded-xl border border-amber-200 p-5 space-y-4 bg-amber-50/50">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <input type="number" name="commission_percent" step="0.01" min="0" max="100" value="{{ old('commission_percent', $coupon->commission_percent) }}" class="w-full rounded-lg border-slate-300">
        ...
        <div class="flex flex-wrap gap-3 pt-2 border-t border-slate-200">
            ...
            <a href="{{ route('admin.coupons.show', $coupon) }}" class="inline-flex items-center gap-2 px-6 py-2.5 bg-slate-200 text-slate-700 rounded-xl font-medium hover:bg-slate-300">عرض التفاصيل</a>
            <a href="{{ route('admin.coupons.index') }}" class="inline-flex items-center gap-2 px-6 py-2.5 text-slate-600 hover:text-slate-800">إلغاء</a>
        </div>
    </form>
</div>
@endsection
