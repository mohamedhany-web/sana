...
<div class="space-y-6">
    <div class="flex flex-wrap items-start justify-between gap-4">
        <div class="flex items-start gap-3">
            <a href="{{ route('admin.coupons.index') }}" class="p-2 rounded-lg hover:bg-slate-100 text-slate-500 transition-colors"><i class="fas fa-arrow-right"></i></a>
            <div>
                <h1 class="text-2xl font-bold text-slate-800 font-heading">
                    <i class="fas fa-ticket-alt text-violet-500 ml-2"></i>{{ $coupon->code }}
                </h1>
                <p class="text-slate-600 mt-1">{{ $coupon->title ?? $coupon->name }}</p>
                <div class="flex flex-wrap gap-2 mt-3">
                    @php
                        $isActive = $coupon->is_active && (!$coupon->expires_at || $coupon->expires_at >= now());
                    @endphp
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $isActive ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600' }}">
        ...
        <div class="flex flex-wrap gap-2">
            <a href="{{ route('admin.coupon-commissions.index', array_filter(['beneficiary_id' => $coupon->beneficiary_user_id])) }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-xl border border-amber-200 text-amber-800 text-sm font-semibold hover:bg-amber-50">
    ...
    <div class="grid md:grid-cols-2 gap-5">
        <div class="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
            ...
            <dl class="space-y-3 text-sm">
                <div class="flex justify-between gap-4"><dt class="text-slate-500">نوع الخصم</dt><dd class="font-medium text-slate-800">{{ $coupon->discount_type === 'percentage' ? 'نسبة مئوية' : 'مبلغ ثابت' }}</dd></div>
                <div class="flex justify-between gap-4"><dt class="text-slate-500">القيمة</dt><dd class="font-bold text-slate-800">{{ $coupon->discount_type === 'percentage' ? $coupon->discount_value.'%' : number_format($coupon->discount_value, 2). currency_suffix() }}</dd></div>
    ...
    <div class="grid md:grid-cols-2 gap-5">
        <div class="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
            ...
            <dl class="space-y-3 text-sm">
                <div class="flex justify-between gap-4"><dt class="text-slate-500">نطاق الكورسات</dt><dd class="font-medium text-slate-800">{{ $scopeLabel }}</dd></div>
                ...
                <div>
                    ...
                    <dd class="text-slate-700 space-y-1">
                        ...
                        <div class="text-xs font-mono bg-slate-100 rounded px-2 py-1">{{ $sc->title }} <span class="text-slate-500">#{{ $sc->id }}</span></div>
                        @endforeach
                    </dd>
                </div>
                @endif
        ...
        <div class="bg-white rounded-xl border border-amber-200 p-6 shadow-sm">
            ...
            <dl class="space-y-3 text-sm">
                <div class="flex justify-between gap-4"><dt class="text-slate-500">المستفيد</dt><dd class="font-medium text-slate-800">{{ $coupon->beneficiary->name ?? '—' }} <span class="font-mono text-xs text-slate-500">#{{ $coupon->beneficiary_user_id }}</span></dd></div>
    ...
    <div class="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
        ...
        <p class="text-slate-600 text-sm leading-relaxed">{{ $coupon->description }}</p>
    </div>
    @endif
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50">
                        <td class="px-4 py-3 text-slate-800">{{ $usage->user->name ?? '—' }}</td>
                        <td class="px-4 py-3 font-mono">
                            @if($usage->order_id)
                            <a href="{{ route('admin.orders.show', $usage->order_id) }}" class="text-violet-600 hover:underline">#{{ $usage->order_id }}</a>
                            @else
                            —
                            @endif
                        </td>
