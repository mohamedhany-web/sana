...
<div class="space-y-6 sm:space-y-8">
    ...
    <section class="rounded-3xl bg-white/95 backdrop-blur border border-slate-200 shadow-lg overflow-hidden">
        <div class="px-5 py-6 sm:px-8 lg:px-12 border-b border-slate-200">
            <nav class="text-sm text-slate-500 mb-4">
                <a href="{{ route('admin.tasks.index') }}" class="hover:text-sky-600 transition-colors">مهام المدربين</a>
                <span class="mx-2">/</span>
                <span class="text-slate-800 font-semibold">{{ $task->title }}</span>
            </nav>
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h1 class="text-2xl sm:text-3xl font-bold text-slate-900">{{ $task->title }}</h1>
                ...
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    @endif
                    <a href="{{ route('admin.tasks.edit', $task) }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold text-slate-700 bg-slate-100 hover:bg-slate-200 border border-slate-200 transition-all">
                        <i class="fas fa-edit"></i>
                        تعديل
                    </a>
                    <a href="{{ route('admin.tasks.index') }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold text-white bg-sky-600 hover:bg-sky-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-sky-500 transition-all">
        ...
        {{-- بطاقات الحالة والتاريخ والتقدم --}}
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 p-5 sm:p-8">
            <div class="rounded-2xl border border-slate-200 bg-slate-50/80 p-5">
                ...
                @php $sb = $statusBadges[$task->status] ?? null; @endphp
                @if($sb)
                    <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold {{ $sb['classes'] }}">
            ...
            <div class="rounded-2xl border border-slate-200 bg-slate-50/80 p-5">
                ...
                @php $pb = $priorityBadges[$task->priority] ?? null; @endphp
                @if($pb)
                    <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold {{ $pb['classes'] }}">
                        <span class="h-2 w-2 rounded-full bg-current"></span>
                        {{ $pb['label'] }}
                    </span>
                @else
            ...
            <div class="rounded-2xl border border-slate-200 bg-slate-50/80 p-5">
                ...
                <p class="text-slate-900 font-bold text-xl">{{ (int)($task->progress ?? 0) }}%</p>
            </div>
            @endif
        </div>
        ...
        @if($task->relatedCourse || $task->relatedLecture)
            <div class="px-5 pb-6 sm:px-8 sm:pb-8">
                <h3 class="text-lg font-bold text-slate-900 mb-2 flex items-center gap-2">
                ...
                <p class="text-slate-700">
                    @if($task->relatedCourse)
                        <a href="{{ route('admin.advanced-courses.show', $task->relatedCourse) }}" class="text-sky-600 hover:text-sky-800 font-medium underline">{{ $task->relatedCourse->title }}</a>
                    @elseif($task->relatedLecture)
                        <a href="{{ route('admin.lectures.show', $task->relatedLecture) }}" class="text-sky-600 hover:text-sky-800 font-medium underline">{{ $task->relatedLecture->title }}</a>
                    @else
                        ID: {{ $task->related_id }}
                    @endif
                </p>
    ...
    {{-- تسليمات المدرب --}}
    @if($task->assigned_by && $task->deliverables->count() > 0)
    <section class="rounded-3xl bg-white/95 backdrop-blur border border-slate-200 shadow-lg overflow-hidden">
        <div class="px-5 py-6 sm:px-8 lg:px-12 border-b border-slate-200">
        ...
        <div class="p-5 sm:p-8 space-y-6">
            ...
            <div class="rounded-2xl border border-slate-200 bg-slate-50/50 p-5 sm:p-6">
                <div class="flex flex-wrap items-start justify-between gap-3 mb-4">
                    <h3 class="text-lg font-bold text-slate-900">{{ $d->title }}</h3>
                    <div class="flex items-center gap-2">
                        <span class="text-xs text-slate-500">{{ $d->submitted_at?->format('Y-m-d H:i') }}</span>
                        @php $dsb = $deliverableStatusBadges[$d->status] ?? ['label' => $d->status, 'classes' => 'bg-slate-100 text-slate-700']; @endphp
                        <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold {{ $dsb['classes'] }}">{{ $dsb['label'] }}</span>
                    </div>
                </div>
                @if($d->description)
                    <p class="text-slate-700 text-sm mb-3">{{ $d->description }}</p>
                @endif
                @if($d->delivery_type === 'link' && $d->link_url)
                    <p class="text-sm mb-2">
                        <a href="{{ $d->link_url }}" target="_blank" class="text-sky-600 hover:text-sky-800 font-medium underline">فتح الرابط</a>
                    </p>
                @endif
                @if($d->file_path)
                    <p class="text-sm mb-2">
                        <a href="{{ \Illuminate\Support\Facades\Storage::url($d->file_path) }}" target="_blank" class="text-sky-600 hover:text-sky-800 font-medium underline">تحميل الملف: {{ $d->file_name ?? 'ملف' }}</a>
                    </p>
                @endif
                @if($d->feedback)
                    <div class="rounded-xl bg-amber-50 border border-amber-200 p-3 mb-4">
                        ...
                        <p class="text-amber-900 text-sm">{{ $d->feedback }}</p>
                    </div>
                @endif
                <form action="{{ route('admin.tasks.review-deliverable', [$task, $d]) }}" method="POST" class="flex flex-wrap items-end gap-3">
                    @csrf
                    ...
                    <div>
                        ...
                        <select name="status" required class="rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-900 focus:ring-2 focus:ring-sky-500 focus:border-sky-400 transition-all">
                            <option value="approved" {{ $d->status === 'approved' ? 'selected' : '' }}>اعتماد</option>
                            <option value="rejected" {{ $d->status === 'rejected' ? 'selected' : '' }}>رفض</option>
                            <option value="needs_revision" {{ $d->status === 'needs_revision' ? 'selected' : '' }}>يحتاج مراجعة</option>
                        </select>
                    </div>
        ...
        <div class="p-5 sm:p-8">
            <div class="space-y-4 mb-6">
                ...
                <div class="rounded-2xl border border-slate-200 bg-slate-50/50 p-4">
                    ...
                    <p class="text-slate-700 text-sm">{{ $comment->comment }}</p>
                </div>
                @empty
