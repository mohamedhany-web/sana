...
<div class="space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-4">
        ...
        <a href="{{ route('admin.referral-programs.index') }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl border border-slate-200 text-slate-700 text-sm font-semibold hover:bg-slate-50">
    ...
    <form method="GET" action="{{ route('admin.referrals.index') }}" class="bg-white rounded-xl border border-slate-200 p-4 flex flex-wrap items-end gap-3">
        ...
        <a href="{{ route('admin.referrals.index') }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg border border-slate-300 text-slate-600 text-sm">إعادة</a>
        @endif
    </form>
    ...
    <div class="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50">
                        <td class="px-4 py-3">
                            <div class="font-medium text-slate-900">{{ $referral->referrer->name ?? '—' }}</div>
                            <div class="text-xs text-slate-500">{{ $referral->referrer->phone ?? '' }}</div>
                        </td>
                        <td class="px-4 py-3">
                            <div class="font-medium text-slate-900">{{ $referral->referred->name ?? '—' }}</div>
                            <div class="text-xs text-slate-500">{{ $referral->referred->phone ?? '' }}</div>
                        </td>
                        <td class="px-4 py-3 text-slate-600">{{ $referral->referralProgram->name ?? '—' }}</td>
                        <td class="px-4 py-3 font-mono text-xs">{{ $referral->referral_code ?? $referral->code ?? '—' }}</td>
                        <td class="px-4 py-3">
                            @php $st = $referral->status; @endphp
                            <span class="inline-flex px-2 py-0.5 rounded-full text-xs font-medium
                                @if($st === 'completed') bg-emerald-100 text-emerald-700
        ...
        @if($referrals->hasPages())<div class="px-4 py-3 border-t border-slate-100">{{ $referrals->links() }}</div>@endif
    </div>
    @else
