...
<div class="md:col-span-2 rounded-2xl border border-slate-200 bg-slate-50/80 p-5 space-y-4"
    ...
    <div>
        <p class="text-sm font-bold text-slate-800 mb-1">{{ __('طريقة عرض السعر على بطاقة الكورس') }}</p>
        <p class="text-xs text-slate-500">{{ __('اختر إما إظهار السعر للشراء المباشر، أو زر واتساب للتواصل مع الدعم الفني.') }}</p>
    </div>
    ...
    <div class="flex flex-col sm:flex-row gap-3">
        <label class="flex-1 flex items-start gap-3 p-4 rounded-xl border-2 cursor-pointer transition-colors"
            ...
            <span>
                <span class="block text-sm font-bold text-slate-800">{{ __('عرض السعر') }}</span>
                <span class="block text-xs text-slate-500 mt-1">{{ __('يظهر السعر على البطاقة ويمكن الشراء من الموقع.') }}</span>
            </span>
        </label>
        <label class="flex-1 flex items-start gap-3 p-4 rounded-xl border-2 cursor-pointer transition-colors"
            ...
            <span>
                <span class="block text-sm font-bold text-slate-800">
                    ...
                    {{ __('التواصل مع الدعم (واتساب)') }}
                </span>
                <span class="block text-xs text-slate-500 mt-1">{{ __('يُخفى السعر ويظهر زر واتساب على بطاقة الكورس وصفحة التفاصيل.') }}</span>
            </span>
        </label>
    </div>
    ...
    <div x-show="pricingMode === 'price'" x-cloak class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div class="space-y-2">
            <label class="block text-sm font-semibold text-slate-700">{{ __('سعر الكورس الأساسي (قبل الخصم') }} — {{ __('public.currency_name') }})</label>
        ...
        <div class="space-y-2">
            <label class="block text-sm font-semibold text-slate-700">{{ __('سعر بعد الخصم (اختياري)') }}</label>
            ...
            <p class="text-xs text-slate-500">{{ __('يُعرض على البطاقات كسعر قبل وبعد. الدفع يُحسب على سعر الشراء الفعلي.') }}</p>
    ...
    <div x-show="pricingMode === 'contact_support'" x-cloak
        ...
        <p>{{ __('سيُستخدم رابط واتساب الدعم من إعدادات الموقع (الفوتر). عند الضغط على البطاقة يفتح واتساب مع رسالة تتضمن اسم الكورس.') }}</p>
    </div>
</div>
