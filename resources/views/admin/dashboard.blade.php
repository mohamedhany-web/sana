...
@section('title', __('لوحة الإدارة - ') . ($platformName ?? config('brand.name', config('app.name'))))
@section('page_title', __('لوحة الإدارة'))
...
<div class="admin-dashboard space-y-7">
    ...
    <div class="admin-dashboard-hero animate-fade-in">
        <div class="admin-dashboard-hero-inner flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            <div>
                ...
                <h2 class="hero-title text-2xl sm:text-3xl font-heading font-bold">{{ __('مرحباً،') }} {{ \App\Support\PlatformBranding::greetingDisplayName(auth()->user()->name) }}</h2>
                <p class="hero-sub text-sm mt-2 max-w-xl leading-relaxed">
                    {{ __('نظرة عامة على منصة :name — الإحصائيات والأقسام أدناه حسب صلاحيات حسابك.', ['name' => $platformName ?? config('brand.name', config('app.name'))]) }}
                </p>
            </div>
            <div class="flex flex-wrap gap-2 shrink-0">
                <a href="{{ route('home') }}" target="_blank" rel="noopener"
                   class="admin-dashboard-hero__btn admin-dashboard-hero__btn--ghost">
                    <i class="fas fa-globe"></i> {{ __('الموقع العام') }}
                </a>
                ...
                <a href="{{ route('admin.system-settings.edit') }}"
                   class="admin-dashboard-hero__btn admin-dashboard-hero__btn--solid">
                    <i class="fas fa-sliders-h"></i> {{ __('الإعدادات') }}
                </a>
                @endif
            </div>
        </div>
    ...
    <div class="dash-quick-panel animate-fade-in animate-fade-in-1">
        ...
        <a href="{{ route('admin.advanced-courses.index') }}" class="admin-quick-link">
            ...
            <span class="text-xs font-bold">{{ __('الكورسات') }}</span>
        </a>
        @endif
        ...
        <a href="{{ route('admin.users.index') }}" class="admin-quick-link">
            ...
            <span class="text-xs font-bold">{{ __('المستخدمون') }}</span>
        </a>
        @endif
        ...
        <a href="{{ route('admin.online-enrollments.index') }}" class="admin-quick-link">
            ...
            <span class="text-xs font-bold">{{ __('التسجيلات') }}</span>
        </a>
        @endif
        ...
        <a href="{{ route('admin.orders.index') }}" class="admin-quick-link">
            ...
            <span class="text-xs font-bold">{{ __('الطلبات') }}</span>
        </a>
        @endif
        ...
        <a href="{{ route('admin.contact-messages.index') }}" class="admin-quick-link">
            ...
            <span class="text-xs font-bold">{{ __('الرسائل') }}</span>
        </a>
        @endif
        <a href="{{ route('admin.notifications.inbox') }}" class="admin-quick-link">
            ...
            <span class="text-xs font-bold">{{ __('الإشعارات') }}</span>
        </a>
        </div>
    </div>
    ...
    @if(isset($dashboardShow) && ! $hasAnyDashboardWidget)
        <div class="rounded-xl border border-amber-200 bg-amber-50/80 px-4 py-3 text-sm text-amber-900">
            ...
            {{ __('لا توجد بطاقات إحصائية لعرضها حالياً. اطلب من المسؤول إسناد الصلاحيات المناسبة لدورك (مثل الكورسات، الطلبات، الفواتير، …).') }}
        </div>
    @endif
    ...
    <section class="dash-section animate-fade-in">
        @include('admin.dashboard.partials.section-heading', [
            ...
            'subtitle' => __('المستخدمون، الطلاب، المدربون، والكورسات'),
    ...
    <div class="dash-grid dash-grid--metrics">
        ...
        <div class="stat-card animate-fade-in animate-fade-in-1" style="--before-bg: #6366f1;">
            <div class="flex items-start justify-between">
                <div class="flex-1 min-w-0">
                    <p class="text-sm font-medium text-slate-500 mb-1">{{ __('إجمالي المستخدمين') }}</p>
            ...
            <div class="mt-3 flex items-center gap-2 text-sm">
                <span class="text-slate-400">{{ __('هذا الشهر:') }}</span>
        ...
        <div class="stat-card animate-fade-in animate-fade-in-2">
            <div class="flex items-start justify-between">
                <div class="flex-1 min-w-0">
                    <p class="text-sm font-medium text-slate-500 mb-1">{{ __('الطلاب') }}</p>
        ...
        <div class="stat-card animate-fade-in animate-fade-in-3">
            <div class="flex items-start justify-between">
                <div class="flex-1 min-w-0">
                    <p class="text-sm font-medium text-slate-500 mb-1">{{ __('المدربين') }}</p>
        ...
        <div class="stat-card animate-fade-in animate-fade-in-4">
            <div class="flex items-start justify-between">
                <div class="flex-1 min-w-0">
                    <p class="text-sm font-medium text-slate-500 mb-1">{{ __('الكورسات') }}</p>
    ...
    <section class="dash-section animate-fade-in">
        @include('admin.dashboard.partials.section-heading', [
            ...
            'subtitle' => __('الإيرادات، الفواتير، والتسجيلات النشطة'),
            'icon' => 'fas fa-coins',
            'tone' => 'emerald',
        ])
    <div class="dash-grid dash-grid--metrics">
        ...
        <div class="stat-card animate-fade-in">
            <div class="flex items-start justify-between">
                <div class="flex-1 min-w-0">
                    <p class="text-sm font-medium text-slate-500 mb-1">{{ __('إجمالي الإيرادات') }}</p>
        ...
        <div class="stat-card animate-fade-in">
            <div class="flex items-start justify-between">
                <div class="flex-1 min-w-0">
                    <p class="text-sm font-medium text-slate-500 mb-1">{{ __('إيرادات الشهر') }}</p>
        ...
        <div class="stat-card animate-fade-in">
            <div class="flex items-start justify-between">
                <div class="flex-1 min-w-0">
                    <p class="text-sm font-medium text-slate-500 mb-1">{{ __('فواتير معلقة') }}</p>
        ...
        <div class="stat-card animate-fade-in">
            <div class="flex items-start justify-between">
                <div class="flex-1 min-w-0">
                    <p class="text-sm font-medium text-slate-500 mb-1">{{ __('التسجيلات النشطة') }}</p>
    ...
    <section class="dash-section">
        @include('admin.dashboard.partials.section-heading', [
            'title' => __('النشاط والامتحانات'),
            'subtitle' => __('آخر التحديثات على المنصة'),
            'icon' => 'fas fa-bolt',
            'tone' => 'violet',
        ])
    <div class="dash-grid dash-grid--panels">
        ...
        <div class="section-card animate-fade-in">
            <div class="section-card-header">
                <h3 class="text-base font-heading font-bold text-slate-800 flex items-center gap-2">
                    <i class="fas fa-history text-indigo-500"></i> {{ __('آخر النشاطات') }}
                </h3>
            </div>
            <div class="dash-list">
                @if(isset($stats['recent_activities']) && $stats['recent_activities']->count() > 0)
                    @foreach($stats['recent_activities']->take(5) as $activity)
                        <div class="list-row">
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-semibold text-slate-700 truncate">{{ $activity->user->name ?? __('مستخدم محذوف') }}</p>
                    ...
                    <div class="section-card-footer-link">
                        <a href="{{ route('admin.activity-log') }}" class="text-sm font-semibold text-indigo-600 hover:text-indigo-700 flex items-center gap-1.5 transition-colors">
                            {{ __('عرض جميع النشاطات') }} <i class="fas fa-arrow-left text-[10px]"></i>
                        </a>
                    </div>
                @else
                    <div class="dash-empty">
                        <i class="fas fa-history"></i>
                        <p class="text-sm">{{ __('لا توجد أنشطة بعد') }}</p>
                    </div>
                @endif
            </div>
        </div>
        ...
        <div class="section-card animate-fade-in">
            <div class="section-card-header">
                <h3 class="text-base font-heading font-bold text-slate-800 flex items-center gap-2">
                    <i class="fas fa-clipboard-check text-violet-500"></i> {{ __('آخر محاولات الامتحانات') }}
                </h3>
            </div>
            <div class="dash-list">
                @if(isset($stats['recent_exam_attempts']) && $stats['recent_exam_attempts']->count() > 0)
                    @foreach($stats['recent_exam_attempts']->take(5) as $attempt)
                        <div class="list-row">
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-semibold text-slate-700">{{ $attempt->student->name ?? __('طالب محذوف') }}</p>
                                <p class="text-xs text-slate-400">{{ $attempt->exam->title ?? __('امتحان محذوف') }}</p>
                            </div>
                ...
                @else
                    <div class="dash-empty">
                        ...
                        <p class="text-sm">{{ __('لا توجد محاولات امتحانات بعد') }}</p>
                    </div>
                @endif
            </div>
        </div>
    ...
    <section class="dash-section">
        @include('admin.dashboard.partials.section-heading', [
            'title' => __('آخر الإضافات'),
            'subtitle' => __('المستخدمون والكورسات المسجّلة حديثاً'),
            'icon' => 'fas fa-clock',
            'tone' => 'amber',
        ])
    <div class="dash-grid dash-grid--panels">
        ...
        <div class="section-card animate-fade-in">
            <div class="section-card-header">
                <h3 class="text-base font-heading font-bold text-slate-800 flex items-center gap-2">
                    <i class="fas fa-users text-sky-500"></i> {{ __('آخر المستخدمين') }}
                </h3>
                <a href="{{ route('admin.users.index') }}" class="text-sm font-semibold text-indigo-600 hover:text-indigo-700 flex items-center gap-1.5 transition-colors">
                    {{ __('عرض الكل') }} <i class="fas fa-arrow-left text-[10px]"></i>
                </a>
            </div>
            <div class="dash-list">
                ...
                <div class="list-row">
                    ...
                    <div class="text-left flex-shrink-0">
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold
                            ...
                            @if($user->role === 'student') {{ __('طالب') }}
                            @elseif($user->role === 'instructor') {{ __('مدرب') }}
                            @elseif($user->role === 'super_admin') {{ __('مدير عام') }}
                            @else {{ __('غير محدد') }} @endif
                        </span>
        ...
        <div class="section-card animate-fade-in">
            <div class="section-card-header">
                <h3 class="text-base font-heading font-bold text-slate-800 flex items-center gap-2">
                    <i class="fas fa-book text-amber-500"></i> {{ __('آخر الكورسات') }}
                </h3>
            ...
            <div class="dash-list">
                ...
                <div class="list-row">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-xs text-slate-400">{{ $course->academicSubject->name ?? __('غير محدد') }}</p>
                    </div>
                    <div class="text-left flex-shrink-0">
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold
                            @if($course->is_active) bg-emerald-50 text-emerald-600
                            @else bg-slate-100 text-slate-500 @endif">
                            @if($course->is_active) {{ __('نشط') }} @else {{ __('غير نشط') }} @endif
                        </span>
                ...
                <div class="dash-empty">
                    <i class="fas fa-book"></i>
                    <p class="text-sm">{{ __('لا توجد كورسات بعد') }}</p>
                </div>
                @endforelse
            </div>
        </div>
    ...
    <section class="dash-section">
        @include('admin.dashboard.partials.section-heading', [
            'title' => __('العمليات الداخلية'),
            'subtitle' => __('المبيعات والموارد البشرية'),
    ...
    <div class="dash-grid dash-grid--panels">
        ...
        <div class="section-card animate-fade-in">
            <div class="section-card-header">
                <h3 class="text-base font-heading font-bold text-slate-800 flex items-center gap-2">
                    <i class="fas fa-shopping-cart text-emerald-500"></i> {{ __('قسم المبيعات') }}
                </h3>
            ...
            <div class="section-card-body section-card-body--padded">
                ...
                <p class="dash-subsection-title">{{ __('آخر الطلبات') }}</p>
                ...
                <div class="list-row">
                    ...
                    <div class="text-left flex-shrink-0">
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold
                            ...
                            @if($order->status === 'approved') {{ __('معتمد') }}
                            @elseif($order->status === 'pending') {{ __('معلق') }}
                            @else {{ __('مرفوض') }} @endif
                        </span>
                ...
                @empty
                <div class="dash-empty"><p class="text-sm">{{ __('لا توجد طلبات حديثة') }}</p></div>
                @endforelse
                </div>
            </div>
        </div>
        ...
        <div class="section-card animate-fade-in">
            <div class="section-card-header">
                <h3 class="text-base font-heading font-bold text-slate-800 flex items-center gap-2">
                    <i class="fas fa-users-cog text-indigo-500"></i> {{ __('قسم الموارد البشرية') }}
                </h3>
            ...
            <div class="section-card-body section-card-body--padded">
                ...
                <div class="list-row">
                    ...
                    <div class="text-left flex-shrink-0">
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold
                            ...
                            @else bg-rose-50 text-rose-600 @endif">
                            @if($leave->status === 'approved') {{ __('معتمد') }}
                            @elseif($leave->status === 'pending') {{ __('معلق') }}
                            @else {{ __('مرفوض') }} @endif
                        </span>
                ...
                @empty
                <div class="dash-empty"><p class="text-sm">{{ __('لا توجد طلبات إجازة حديثة') }}</p></div>
                @endforelse
                </div>
                <p class="dash-subsection-title mt-3">{{ __('آخر الموظفين المضافة') }}</p>
                <div class="dash-list rounded-xl border border-slate-100 overflow-hidden">
                    @forelse($hrSection['recent_employees'] ?? [] as $emp)
                    <div class="list-row">
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-semibold text-slate-700 truncate">{{ $emp->name }}</p>
                            <p class="text-xs text-slate-400">{{ optional($emp->employeeJob)->name ?? __('موظف') }}</p>
                        </div>
                        <p class="text-[11px] text-slate-400">{{ $emp->hire_date ? $emp->hire_date->diffForHumans() : $emp->created_at->diffForHumans() }}</p>
                    </div>
                    @empty
                    <div class="dash-empty"><p class="text-sm">{{ __('لا يوجد موظفون') }}</p></div>
                    @endforelse
                </div>
            </div>
        </div>
    ...
    <div class="dash-grid dash-grid--panels">
        ...
        <div class="section-card animate-fade-in">
            <div class="section-card-header">
                <h3 class="text-base font-heading font-bold text-slate-800 flex items-center gap-2">
                    <i class="fas fa-file-invoice text-amber-500"></i> {{ __('الفواتير المعلقة') }}
                </h3>
                <a href="#" class="text-sm font-semibold text-indigo-600 hover:text-indigo-700 flex items-center gap-1.5 transition-colors">{{ __('عرض الكل') }} <i class="fas fa-arrow-left text-[10px]"></i></a>
            </div>
            <div class="dash-list">
                ...
                <div class="list-row">
                    ...
                    <div class="flex-1 min-w-0">
                        <p class="text-sm font-semibold text-slate-700 truncate">{{ $invoice->invoice_number ?? __('غير محدد') }}</p>
                        <p class="text-xs text-slate-400">{{ $invoice->user->name ?? __('غير محدد') }}</p>
                    </div>
                    <div class="text-left flex-shrink-0">
        ...
        <div class="section-card animate-fade-in">
            <div class="section-card-header">
                <h3 class="text-base font-heading font-bold text-slate-800 flex items-center gap-2">
                    <i class="fas fa-money-bill-wave text-emerald-500"></i> {{ __('المدفوعات الأخيرة') }}
                </h3>
                <a href="#" class="text-sm font-semibold text-indigo-600 hover:text-indigo-700 flex items-center gap-1.5 transition-colors">{{ __('عرض الكل') }} <i class="fas fa-arrow-left text-[10px]"></i></a>
            </div>
            <div class="dash-list">
                ...
                <div class="list-row">
                    ...
                    <div class="flex-1 min-w-0">
                        <p class="text-sm font-semibold text-slate-700 truncate">{{ $payment->payment_number ?? __('غير محدد') }}</p>
                        <p class="text-xs text-slate-400">{{ $payment->user->name ?? __('غير محدد') }}</p>
                    </div>
                    <div class="text-left flex-shrink-0">
    ...
    <div class="section-card animate-fade-in">
        <div class="section-card-header">
            <h3 class="text-base font-heading font-bold text-slate-800 flex items-center gap-2">
                <i class="fas fa-tags text-sky-500"></i> {{ __('الباقات والأسعار — الاشتراكات') }}
            </h3>
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('admin.pricing-packages.index') }}" class="text-sm font-semibold text-sky-600 hover:text-sky-700 flex items-center gap-1.5 transition-colors">
                    {{ __('قسم الباقات والأسعار') }} <i class="fas fa-arrow-left text-[10px]"></i>
                </a>
                @endif
                <a href="{{ route('admin.subscriptions.index') }}" class="text-sm font-semibold text-indigo-600 hover:text-indigo-700 flex items-center gap-1.5 transition-colors">
                    {{ __('عرض كل الاشتراكات') }} <i class="fas fa-arrow-left text-[10px]"></i>
                </a>
        ...
        <div class="p-5">
            ...
            <p class="text-xs text-slate-500 mb-4">{{ __('الباقات المتاحة وجميع المشتركين في كل باقة. يمكنك الدخول على أي مشترك لإدارة اشتراكه ومراجعة بياناته بالكامل.') }}</p>
                ...
                <div class="rounded-2xl border border-slate-200 bg-slate-50/50 overflow-hidden">
                    ...
                    <div class="p-3 max-h-64 overflow-y-auto space-y-1.5">
                        ...
                        <div class="flex items-center justify-between gap-2 rounded-xl border border-slate-100 bg-white px-3 py-2 hover:border-indigo-200 transition-colors">
                            ...
                            <div class="flex items-center gap-1 flex-shrink-0">
                                <a href="{{ route('admin.subscriptions.show', $sub) }}" class="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-sky-100 text-sky-600 hover:bg-sky-200 transition-colors text-xs" title="{{ __('تفاصيل الاشتراك والتحكم') }}">
                                    <i class="fas fa-eye"></i>
                                </a>
                                @if($sub->user)
                                <a href="{{ route('admin.users.show', $sub->user->id) }}" class="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-slate-100 text-slate-600 hover:bg-slate-200 transition-colors text-xs" title="{{ __('بيانات المستخدم') }}">
                        ...
                        @empty
                        <p class="text-xs text-slate-400 py-2 text-center">{{ __('لا مشتركين في هذه الباقة') }}</p>
                        @endforelse
                    </div>
                    ...
                    <div class="px-4 py-2 border-t border-slate-100 bg-slate-50/80 text-center">
                        <a href="{{ route('admin.subscriptions.index') }}" class="text-xs font-semibold text-indigo-600 hover:text-indigo-700">
                            {{ __('عرض كل الاشتراكات') }} ({{ $package['count'] }} {{ __('في هذه الباقة') }}) <i class="fas fa-arrow-left text-[10px]"></i>
                        </a>
                    </div>
                    @endif
                </div>
            ...
            <div class="py-10 text-center rounded-xl bg-slate-50 border border-slate-100">
                ...
                <p class="text-sm text-slate-500">{{ __('لا توجد اشتراكات أو باقات حالياً.') }}</p>
                <a href="{{ route('admin.subscriptions.create') }}" class="inline-flex items-center gap-2 mt-3 text-sm font-semibold text-indigo-600 hover:text-indigo-700">
                    <i class="fas fa-plus"></i> {{ __('إضافة اشتراك جديد') }}
                </a>
            </div>
            @endif
        </div>
    ...
    <div class="section-card animate-fade-in">
        <div class="section-card-header">
            <div>
                <h3 class="text-base font-heading font-bold text-slate-800 flex items-center gap-2">
                    <i class="fas fa-bolt text-amber-500"></i> {{ __('إجراءات سريعة') }}
                </h3>
                <p class="text-xs text-slate-400 mt-0.5">{{ __('روابط مباشرة للمهام اليومية') }}</p>
            </div>
        </div>
        <div class="section-card-body section-card-body--padded">
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
                ...
                @if(empty($quickActions))
                    <div class="col-span-full dash-empty">
                        ...
                        <p class="text-sm">{{ __('لا توجد مهام عاجلة حالياً') }}</p>
                    </div>
                @endif
            </div>
        </div>
