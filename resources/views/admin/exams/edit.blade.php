        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="min-w-0">
                <nav class="text-sm text-white/80 mb-2">
                    ...
                    <span class="mx-2">/</span>
                    <span class="text-white">{{ __('تحرير') }}</span>
                </nav>
                <h1 class="text-xl sm:text-2xl font-bold mt-1">{{ __('تحرير الامتحان') }}</h1>
    ...
    <form action="{{ route('admin.exams.update', $exam) }}" method="POST" class="space-y-6">
        ...
        <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
            ...
            <div class="xl:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                    ...
                    <div class="p-6 space-y-6">
                        <div>
                            <label for="title" class="block text-sm font-semibold text-gray-700 mb-2">{{ __('عنوان الامتحان') }} <span class="text-red-500">*</span></label>
                        ...
                        <div>
                            <label for="advanced_course_id" class="block text-sm font-semibold text-gray-700 mb-2">{{ __('الكورس') }} <span class="text-red-500">*</span></label>
                        ...
                        <div>
                            <label for="course_lesson_id" class="block text-sm font-semibold text-gray-700 mb-2">{{ __('الدرس (اختياري)') }}</label>
                            <select name="course_lesson_id" id="course_lesson_id"
                                    class="w-full px-4 py-2.5 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                                <option value="">{{ __('لا يوجد') }}</option>
                        ...
                        <div>
                            <label for="description" class="block text-sm font-semibold text-gray-700 mb-2">{{ __('الوصف') }}</label>
                        ...
                        <div>
                            <label for="instructions" class="block text-sm font-semibold text-gray-700 mb-2">{{ __('تعليمات الامتحان') }}</label>
                ...
                <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                    ...
                    <div class="p-6 space-y-6">
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <div>
                                <label for="duration_minutes" class="block text-sm font-semibold text-gray-700 mb-2">{{ __('مدة الامتحان (دقيقة)') }} <span class="text-red-500">*</span></label>
                            ...
                            <div>
                                <label for="attempts_allowed" class="block text-sm font-semibold text-gray-700 mb-2">{{ __('المحاولات المسموحة') }} <span class="text-red-500">*</span></label>
                                ...
                                <p class="mt-1 text-xs text-gray-500">0 = {{ __('غير محدود') }}</p>
                                @error('attempts_allowed')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                            </div>
                            <div>
                                <label for="passing_marks" class="block text-sm font-semibold text-gray-700 mb-2">{{ __('درجة النجاح (%)') }} <span class="text-red-500">*</span></label>
                            ...
                            <div>
                                <label for="total_marks" class="block text-sm font-semibold text-gray-700 mb-2">{{ __('إجمالي الدرجات') }}</label>
                ...
                <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                    ...
                    <div class="p-6">
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <div>
                                <label for="start_time" class="block text-sm font-semibold text-gray-700 mb-2">{{ __('وقت البداية') }}</label>
                                <input type="datetime-local" name="start_time" id="start_time" value="{{ $startTime }}"
                                       class="w-full px-4 py-2.5 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                                <p class="mt-1 text-xs text-gray-500">{{ __('فارغ = متاح فوراً') }}</p>
                                @error('start_time')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                            </div>
                            <div>
                                <label for="end_time" class="block text-sm font-semibold text-gray-700 mb-2">{{ __('وقت النهاية') }}</label>
                                <input type="datetime-local" name="end_time" id="end_time" value="{{ $endTime }}"
                                       class="w-full px-4 py-2.5 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                                <p class="mt-1 text-xs text-gray-500">{{ __('فارغ = متاح باستمرار') }}</p>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="space-y-3">
                                <label class="flex items-center gap-3 p-3 rounded-xl border border-gray-200 hover:bg-gray-50 cursor-pointer">
                                    ...
                                    <span class="text-sm font-medium text-gray-800">{{ __('خلط ترتيب الأسئلة') }}</span>
                                </label>
                                <label class="flex items-center gap-3 p-3 rounded-xl border border-gray-200 hover:bg-gray-50 cursor-pointer">
                                    ...
                                    <span class="text-sm font-medium text-gray-800">{{ __('خلط خيارات الإجابة') }}</span>
                                </label>
                                <label class="flex items-center gap-3 p-3 rounded-xl border border-gray-200 hover:bg-gray-50 cursor-pointer">
                                    ...
                                    <span class="text-sm font-medium text-gray-800">{{ __('عرض النتائج فور الانتهاء') }}</span>
                                </label>
                                <label class="flex items-center gap-3 p-3 rounded-xl border border-gray-200 hover:bg-gray-50 cursor-pointer">
                                    ...
                                    <span class="text-sm font-medium text-gray-800">{{ __('السماح بمراجعة الأسئلة والإجابات') }}</span>
                                </label>
                            </div>
                            <div class="space-y-3">
                                <label class="flex items-center gap-3 p-3 rounded-xl border border-gray-200 hover:bg-gray-50 cursor-pointer">
                                    ...
                                    <span class="text-sm font-medium text-gray-800">{{ __('عرض الإجابات الصحيحة') }}</span>
                                </label>
                                <label class="flex items-center gap-3 p-3 rounded-xl border border-gray-200 hover:bg-gray-50 cursor-pointer">
                                    ...
                                    <span class="text-sm font-medium text-gray-800">{{ __('عرض تفسيرات الإجابات') }}</span>
                                </label>
                                <label class="flex items-center gap-3 p-3 rounded-xl border border-gray-200 hover:bg-gray-50 cursor-pointer">
                                    ...
                                    <span class="text-sm font-medium text-gray-800">{{ __('منع تبديل التبويبات أثناء الامتحان') }}</span>
                                </label>
                                <label class="flex items-center gap-3 p-3 rounded-xl border border-gray-200 hover:bg-gray-50 cursor-pointer">
                                    ...
                                    <span class="text-sm font-medium text-gray-800">{{ __('تسليم تلقائي عند انتهاء الوقت') }}</span>
                                </label>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="p-6">
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <label class="flex items-center gap-3 p-3 rounded-xl border border-gray-200 hover:bg-gray-50 cursor-pointer">
                                ...
                                <span class="text-sm font-medium text-gray-800">{{ __('تتطلب تفعيل الكاميرا') }}</span>
                            </label>
                            <label class="flex items-center gap-3 p-3 rounded-xl border border-gray-200 hover:bg-gray-50 cursor-pointer">
                                ...
                                <span class="text-sm font-medium text-gray-800">{{ __('تتطلب تفعيل الميكروفون') }}</span>
                            </label>
                        </div>
                    </div>
                </div>
            ...
            <div class="space-y-6">
                <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                    <div class="px-6 py-4 bg-gray-50 border-b border-gray-200">
                        <h2 class="text-lg font-bold text-gray-900">{{ __('حالة الامتحان') }}</h2>
                    </div>
                    <div class="p-6">
                        <label class="flex items-center gap-3 p-4 rounded-xl border-2 border-gray-200 hover:border-indigo-200 cursor-pointer">
                            ...
                            <span class="font-semibold text-gray-800">{{ __('امتحان نشط') }}</span>
                        </label>
                        <p class="text-xs text-gray-500 mt-2">{{ __('غير النشط لا يظهر للطلاب') }}</p>
                    </div>
                </div>

                <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                    <div class="px-6 py-4 bg-gray-50 border-b border-gray-200">
                        <h2 class="text-lg font-bold text-gray-900">{{ __('معلومات') }}</h2>
                    </div>
                    <div class="p-6 space-y-3 text-sm">
                        <div class="flex justify-between"><span class="text-gray-500">{{ __('تاريخ الإنشاء') }}</span><span class="font-medium text-gray-800">{{ $exam->created_at->format('Y-m-d H:i') }}</span></div>
                        <div class="flex justify-between"><span class="text-gray-500">{{ __('آخر تحديث') }}</span><span class="font-medium text-gray-800">{{ $exam->updated_at->format('Y-m-d H:i') }}</span></div>
                        <div class="flex justify-between"><span class="text-gray-500">{{ __('الأسئلة') }}</span><span class="font-medium text-gray-800">{{ $exam->examQuestions->count() }}</span></div>
                        <div class="flex justify-between"><span class="text-gray-500">{{ __('المحاولات') }}</span><span class="font-medium text-gray-800">{{ $exam->attempts->count() }}</span></div>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    courseSelect.addEventListener('change', function() {
        ...
        lessonSelect.innerHTML = '<option value="">{{ __('جاري التحميل...') }}</option>';

        if (!courseId) {
            lessonSelect.innerHTML = '<option value="">{{ __('لا يوجد') }}</option>';
            return;
        }
        ...
        fetch(url, { headers: { 'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest' } })
            ...
            .catch(function() {
                lessonSelect.innerHTML = '<option value="">{{ __('خطأ في التحميل') }}</option>';
            });
    });
});
</script>
