...
@section('title', __('امتحانات: ') . $course->title)
...
<div class="w-full max-w-full px-4 py-6 space-y-6">
    ...
    <div class="bg-gradient-to-l from-indigo-600 via-blue-600 to-cyan-500 rounded-2xl p-6 text-white shadow-lg">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="min-w-0">
                ...
                <p class="text-sm text-white/90 mt-1">{{ __('إدارة امتحانات هذا الكورس — عرض، إضافة، تعديل، حذف، أسئلة، إحصائيات') }}</p>
            </div>
    ...
    @if($exams->count() > 0)
        <div class="bg-white rounded-2xl border border-gray-200 shadow-lg overflow-hidden">
            <div class="px-6 py-4 bg-gray-50 border-b border-gray-200 flex flex-wrap items-center justify-between gap-4">
                <h4 class="text-lg font-bold text-gray-900">{{ __('الامتحانات') }} ({{ $exams->total() }})</h4>
            </div>
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-6 py-3 text-right text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ __('العنوان') }}</th>
                            <th class="px-6 py-3 text-right text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ __('المدة') }}</th>
                            <th class="px-6 py-3 text-right text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ __('الأسئلة') }}</th>
                            <th class="px-6 py-3 text-right text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ __('المحاولات') }}</th>
                            <th class="px-6 py-3 text-right text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ __('الحالة') }}</th>
                            <th class="px-6 py-3 text-right text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ __('الإجراءات') }}</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach($exams as $exam)
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $exam->duration_minutes }} {{ __('د') }}</td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold {{ $exam->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                        {{ $exam->is_active ? __('نشط') : __('معطل') }}
                                    </span>
                                    @if($exam->is_published)
                                        <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-blue-100 text-blue-800 mr-1">{{ __('منشور') }}</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-2 flex-wrap">
                                        ...
                                        <button type="button" onclick="toggleExamStatus({{ $exam->id }})" class="inline-flex items-center justify-center w-9 h-9 rounded-xl {{ $exam->is_active ? 'bg-red-50 text-red-600 hover:bg-red-100' : 'bg-green-50 text-green-600 hover:bg-green-100' }} transition-colors" title="{{ $exam->is_active ? __('إيقاف') : __('تفعيل') }}"><i class="fas {{ $exam->is_active ? 'fa-pause' : 'fa-play' }}"></i></button>
                                        <button type="button" onclick="toggleExamPublish({{ $exam->id }})" class="inline-flex items-center justify-center w-9 h-9 rounded-xl {{ $exam->is_published ? 'bg-yellow-50 text-yellow-600 hover:bg-yellow-100' : 'bg-purple-50 text-purple-600 hover:bg-purple-100' }} transition-colors" title="{{ $exam->is_published ? __('إلغاء النشر') : __('نشر') }}"><i class="fas fa-globe"></i></button>
                                        <form action="{{ route('admin.exams.destroy', $exam) }}" method="POST" class="inline" onsubmit="return confirm(@json(__('هل أنت متأكد من حذف هذا الامتحان؟')));">
                                            @csrf
                                            @method('DELETE')
    ...
    @else
        <div class="bg-white rounded-2xl border border-gray-200 shadow-lg p-12 text-center">
            ...
            <h3 class="text-xl font-bold text-gray-900 mb-2">{{ __('لا توجد امتحانات في هذا الكورس') }}</h3>
            <p class="text-gray-500 mb-6">{{ __('يمكنك إضافة أول امتحان لهذا الكورس') }}</p>
            <a href="{{ route('admin.exams.create', ['course_id' => $course->id]) }}"
...
function toggleExamStatus(examId) {
    if (confirm(@json(__('هل تريد تغيير حالة هذا الامتحان؟')))) {
        fetch('/admin/exams/' + examId + '/toggle-status', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}', 'Accept': 'application/json' }
        })
        .then(function(r) { return r.json(); })
        .then(function(data) { if (data.success) location.reload(); else alert(@json(__('حدث خطأ'))); })
        .catch(function() { alert(@json(__('حدث خطأ'))); });
    }
}
function toggleExamPublish(examId) {
    if (confirm(@json(__('هل تريد تغيير حالة نشر هذا الامتحان؟')))) {
