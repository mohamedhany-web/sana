        ...
        <div class="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
            ...
            <dl class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                <div>
                    ...
                    <dd class="font-medium text-slate-800">{{ $reportView->user?->name ?? '—' }}
                        @if($reportView->user_id)<span class="text-slate-400 text-xs">(#{{ $reportView->user_id }})</span>@endif
                    </dd>
                </div>
                <div>
                    <dt class="text-slate-500">{{ $reportView->source === 'live_session' ? 'الجلسة' : 'الاجتماع' }}</dt>
                    <dd class="font-medium text-slate-800">{{ $reportView->context_title ?? '—' }}
            ...
            <div class="p-6">
                @if($reportView->summary)
                    <div id="report-summary-body"
                         class="text-sm text-slate-800 whitespace-pre-wrap leading-relaxed rounded-lg border border-slate-100 bg-slate-50/80 px-4 py-4 max-h-[70vh] overflow-y-auto">{{ $reportView->summary }}</div>
