...
@section('title', __('تصنيفات دعم الطلاب'))
@section('header', __('تصنيفات دعم الطلاب'))
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <aside class="xl:col-span-1">
            <section class="rounded-2xl bg-white border border-slate-200 shadow-lg overflow-hidden sticky top-4">
                ...
                <form method="POST" action="{{ route('admin.support-inquiry-categories.store') }}" class="p-5 space-y-4" data-turbo="false">
                    @csrf
                    <div>
                        <label for="new_cat_name" class="block text-xs font-bold text-slate-700 mb-1.5">اسم التصنيف</label>
                        <input type="text" id="new_cat_name" name="name" value="{{ old('name') }}" required maxlength="120"
                               placeholder="{{ __('مثال: مشكلة في الحصص المباشرة') }}"
                    ...
                    <div>
                        ...
                        <input type="number" id="new_cat_sort" name="sort_order" value="{{ old('sort_order', 0) }}" min="0" max="9999"
        ...
        <section class="xl:col-span-2 rounded-2xl bg-white border border-slate-200 shadow-lg overflow-hidden">
            ...
            @forelse($categories as $cat)
                <div class="border-b border-slate-100 last:border-b-0 {{ $cat->is_active ? '' : 'bg-slate-50/80' }}">
                    <div class="p-5">
                        <div class="flex flex-wrap items-start justify-between gap-3 mb-4">
                            <div class="flex items-center gap-3 min-w-0">
                                <div class="w-10 h-10 rounded-xl flex items-center justify-center shrink-0 {{ $cat->is_active ? 'bg-indigo-100 text-indigo-600' : 'bg-slate-200 text-slate-500' }}">
                                    <i class="fas fa-folder-open"></i>
                                </div>
                                <div class="min-w-0">
                                    <p class="text-base font-black text-slate-900 truncate">{{ $cat->name }}</p>
                                    <div class="flex flex-wrap items-center gap-2 mt-1">
                                        ...
                                        @endif
                                        <span class="text-[11px] text-slate-500">ترتيب: {{ $cat->sort_order }}</span>
                                    </div>
                                </div>
                            </div>
                            <div class="flex flex-wrap gap-2 text-xs">
                                @if(($cat->active_tickets_count ?? 0) > 0)
                                    <a href="{{ route('admin.support-tickets.index', ['category_id' => $cat->id, 'view' => 'active']) }}"
                                ...
                                <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-slate-100 text-slate-700 font-semibold">
                                    ...
                                    {{ $cat->tickets_count ?? 0 }} إجمالي
                                </span>
                            </div>
                        </div>

                        <form method="POST" action="{{ route('admin.support-inquiry-categories.update', $cat) }}"
                            ...
                            <div class="sm:col-span-6">
                                ...
                                <input type="text" name="name" value="{{ old('name', $cat->name) }}" required maxlength="120"
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <input type="number" name="sort_order" value="{{ old('sort_order', $cat->sort_order) }}" min="0"
                                       class="w-full px-3 py-2 rounded-xl border border-slate-200 text-sm focus:ring-2 focus:ring-indigo-500">
                            </div>
                            <div class="sm:col-span-2 flex items-center pb-2">
                                <label class="inline-flex items-center gap-2 text-sm text-slate-700 cursor-pointer">
                                    <input type="hidden" name="is_active" value="0">
                                    <input type="checkbox" name="is_active" value="1" class="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" {{ $cat->is_active ? 'checked' : '' }}>
                        ...
                        <div class="mt-3 pt-3 border-t border-slate-100 flex justify-end">
                            <form method="POST" action="{{ route('admin.support-inquiry-categories.destroy', $cat) }}" data-turbo="false"
                                  onsubmit="return confirm('حذف «{{ addslashes($cat->name) }}»؟ التذاكر المرتبطة ستبقى دون تصنيف.');">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                                        class="inline-flex items-center gap-1 text-xs font-bold text-rose-600 hover:text-rose-800 disabled:opacity-40"
                                        @if(($cat->active_tickets_count ?? 0) > 0) disabled title="{{ __('أغلق التذاكر النشطة أولاً') }}" @endif>
