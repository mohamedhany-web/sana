...
@section('title', __('رقابة الطالب — ') . $student->name)
...
<div class="w-full space-y-6">
    <nav class="text-sm text-slate-500 flex flex-wrap items-center gap-1">
        <a href="{{ route('admin.quality-control.index') }}" class="text-violet-600 hover:text-violet-800 font-semibold">{{ __('الرقابة والجودة') }}</a>
        <span>/</span>
        <a href="{{ route('admin.quality-control.students') }}" class="text-violet-600 hover:text-violet-800 font-semibold">{{ __('الطلاب') }}</a>
        <span>/</span>
 ...
 {{ __('آخر دخول:') }} {{ $student->last_login_at ? $student->last_login_at->format('Y-m-d H:i') . ' (' . $student->last_login_at->diffForHumans() . ')' : '—' }}
                    </p>
                </div>
            </div>
    ...
    <section class="rounded-2xl bg-white border border-slate-200 shadow-lg overflow-hidden">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 p-5 sm:p-6">
            <div class="rounded-xl border border-violet-200 bg-violet-50/50 p-3">
                <p class="text-[11px] font-bold text-violet-700">{{ __('تسجيلات كورس') }}</p>
                <p class="text-xl font-black text-slate-900">{{ $enrollmentStats['total'] }}</p>
                <p class="text-[10px] text-slate-500">{{ __('نشط') }} {{ $enrollmentStats['active'] }} · مكتمل {{ $enrollmentStats['completed'] }}</p>
            </div>
            <div class="rounded-xl border border-sky-200 bg-sky-50/50 p-3">
                <p class="text-[11px] font-bold text-sky-700">{{ __('حصص مع معلم') }}</p>
            ...
            <div class="rounded-xl border border-indigo-200 bg-indigo-50/50 p-3">
                <p class="text-[11px] font-bold text-indigo-700">{{ __('اختبارات') }}</p>
            ...
            <div class="rounded-xl border border-slate-200 bg-slate-50/50 p-3">
                <p class="text-[11px] font-bold text-slate-600">{{ __('شهادات') }}</p>
    ...
    <section class="rounded-2xl bg-white border border-slate-200 shadow-lg overflow-hidden">
        <div class="px-5 py-4 border-b border-slate-200">
            <h2 class="text-lg font-black text-slate-900">{{ __('البيانات الشخصية') }}</h2>
        </div>
        <div class="p-5 sm:p-8">
            <dl class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-4 text-sm">
                <div><dt class="text-slate-500 text-xs font-semibold mb-0.5">{{ __('الاسم') }}</dt><dd class="font-bold text-slate-900">{{ $student->name }}</dd></div>
                <div><dt class="text-slate-500 text-xs font-semibold mb-0.5">{{ __('البريد') }}</dt><dd class="text-slate-900 break-all">{{ $student->email ?? '—' }}</dd></div>
                <div><dt class="text-slate-500 text-xs font-semibold mb-0.5">{{ __('الجوال') }}</dt><dd class="text-slate-900" dir="ltr">{{ $student->phone ?? '—' }}</dd></div>
                <div><dt class="text-slate-500 text-xs font-semibold mb-0.5">{{ __('السنة الدراسية') }}</dt><dd class="text-slate-900">{{ $student->academicYear->name ?? '—' }}</dd></div>
                <div><dt class="text-slate-500 text-xs font-semibold mb-0.5">{{ __('تاريخ الميلاد') }}</dt><dd class="text-slate-900">{{ $student->birth_date ? $student->birth_date->format('Y-m-d') : '—' }}</dd></div>
                <div><dt class="text-slate-500 text-xs font-semibold mb-0.5">{{ __('الحالة') }}</dt>
                    <dd><span class="inline-flex px-2 py-0.5 rounded-full text-xs font-bold {{ $student->is_active ? 'bg-emerald-100 text-emerald-800' : 'bg-rose-100 text-rose-800' }}">{{ $student->is_active ? __('نشط') : __('معطّل') }}</span></dd>
                </div>
                <div class="md:col-span-2 lg:col-span-3"><dt class="text-slate-500 text-xs font-semibold mb-0.5">{{ __('العنوان') }}</dt><dd class="text-slate-900">{{ $student->address ?? '—' }}</dd></div>
                <div><dt class="text-slate-500 text-xs font-semibold mb-0.5">{{ __('تاريخ التسجيل') }}</dt><dd class="text-slate-900">{{ $student->created_at->format('Y-m-d H:i') }}</dd></div>
                <div><dt class="text-slate-500 text-xs font-semibold mb-0.5">{{ __('آخر تحديث للحساب') }}</dt><dd class="text-slate-900">{{ $student->updated_at->format('Y-m-d H:i') }}</dd></div>
            </dl>
            @if($student->guardians->isNotEmpty())
                <div class="mt-6 pt-6 border-t border-slate-100">
                    <p class="text-xs font-bold text-slate-600 mb-2">{{ __('أولياء الأمور') }}</p>
                    <div class="flex flex-wrap gap-2">
            ...
            @if($student->academicSupervisors->isNotEmpty())
                <div class="mt-4">
                    <p class="text-xs font-bold text-slate-600 mb-2">{{ __('المشرفون الأكاديميون') }}</p>
                    <div class="flex flex-wrap gap-2">
                ...
                <table class="min-w-full divide-y divide-slate-200 text-right text-sm">
                    <thead class="bg-slate-50 text-xs font-bold text-slate-600">
                        <tr>
                            ...
                            <th class="px-4 py-3">{{ __('ساعات حصص') }}</th>
                        </tr>
                    </thead>
    ...
    <section class="rounded-2xl bg-white border border-slate-200 shadow-lg overflow-hidden">
        <div class="px-5 py-4 border-b border-slate-200">
            <h2 class="text-lg font-black text-slate-900">{{ __('حصص مع المعلمين') }}</h2>
        </div>
        @if($profile)
            <div class="px-5 py-4 bg-violet-50/40 border-b border-violet-100 grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
                <div><p class="text-xs text-slate-500">{{ __('رصيد الساعات') }}</p><p class="font-black text-slate-900">{{ (int) $profile->lesson_hours_quota }} س</p></div>
                <div><p class="text-xs text-slate-500">{{ __('المستخدم') }}</p><p class="font-black text-slate-900">{{ (int) $profile->lesson_hours_used }} س</p></div>
                <div><p class="text-xs text-slate-500">{{ __('المتبقي') }}</p><p class="font-black text-emerald-700">{{ max(0, (int) $profile->lesson_hours_quota - (int) $profile->lesson_hours_used) }} س</p></div>
                <div><p class="text-xs text-slate-500">{{ __('نمط المطابقة') }}</p><p class="font-semibold text-slate-800">{{ $matchingLabels[$profile->matching_mode] ?? $profile->matching_mode }}</p></div>
            </div>
        @else
            <p class="px-5 py-3 text-xs text-amber-800 bg-amber-50 border-b border-amber-100">{{ __('لم يُنشأ ملف تعلّم بعد (يُنشأ عادةً مع الاشتراك أو أول حصة).') }}</p>
        @endif
        <div class="overflow-x-auto">
                ...
                <table class="min-w-full divide-y divide-slate-200 text-right text-sm">
                    <thead class="bg-slate-50 text-xs font-bold text-slate-600">
                        <tr>
                            ...
                            <th class="px-4 py-3">{{ __('المعلم') }}</th>
                            <th class="px-4 py-3">{{ __('الحالة') }}</th>
    ...
    <section class="rounded-2xl bg-white border border-slate-200 shadow-lg overflow-hidden">
        <div class="px-5 py-4 border-b border-slate-200">
            <h2 class="text-lg font-black text-slate-900">{{ __('تسجيلات الكورسات (أونلاين)') }}</h2>
        </div>
        <div class="overflow-x-auto">
                ...
                <table class="min-w-full divide-y divide-slate-200 text-right text-sm">
                    <thead class="bg-slate-50 text-xs font-bold text-slate-600">
                        <tr>
                            ...
                            <th class="px-4 py-3">{{ __('المدرب') }}</th>
                            <th class="px-4 py-3">{{ __('الحالة') }}</th>
                            <th class="px-4 py-3">{{ __('التقدّم') }}</th>
                            <th class="px-4 py-3">{{ __('السعر') }}</th>
            ...
            @else
                <p class="p-8 text-center text-sm text-slate-500">{{ __('لا توجد تسجيلات كورسات.') }}</p>
            @endif
        </div>
    </section>
    ...
    <section class="rounded-2xl bg-white border border-slate-200 shadow-lg overflow-hidden">
        <div class="px-5 py-4 border-b border-slate-200">
            <h2 class="text-lg font-black text-slate-900">{{ __('اتفاقيات التقسيط') }}</h2>
        </div>
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-slate-200 text-right text-sm">
                <thead class="bg-slate-50 text-xs font-bold text-slate-600">
    ...
    <section class="rounded-2xl bg-white border border-slate-200 shadow-lg overflow-hidden">
        <div class="px-5 py-4 border-b border-slate-200 flex items-center justify-between">
            <h2 class="text-lg font-black text-slate-900">{{ __('تذاكر الدعم الفني') }}</h2>
            @if(Route::has('admin.support-tickets.index'))
                <a href="{{ route('admin.support-tickets.index') }}" class="text-xs font-bold text-sky-600 hover:underline">{{ __('كل التذاكر') }}</a>
            @endif
        </div>
        <div class="overflow-x-auto">
                            ...
                            <tr class="hover:bg-slate-50">
                                ...
                                <td class="px-4 py-3">
                                    @if(Route::has('admin.support-tickets.show'))
                                        <a href="{{ route('admin.support-tickets.show', $ticket) }}" class="text-xs font-bold text-sky-600">{{ __('فتح') }}</a>
                                    @endif
                                </td>
                            </tr>
                        @endforeach
    ...
    <section class="rounded-2xl bg-white border border-slate-200 shadow-lg overflow-hidden">
        <div class="px-5 py-4 border-b border-slate-200">
            <h2 class="text-lg font-black text-slate-900">{{ __('محاولات الاختبارات') }}</h2>
        </div>
        <div class="overflow-x-auto">
            @if($examAttempts->isNotEmpty())
                <table class="min-w-full divide-y divide-slate-200 text-right text-sm">
                    <thead class="bg-slate-50 text-xs font-bold text-slate-600">
                        <tr>
                            <th class="px-4 py-3">{{ __('الاختبار') }}</th>
                            <th class="px-4 py-3">{{ __('الدرجة') }}</th>
                            <th class="px-4 py-3">{{ __('النسبة') }}</th>
                            <th class="px-4 py-3">{{ __('الحالة') }}</th>
                            <th class="px-4 py-3">{{ __('التاريخ') }}</th>
                        </tr>
                    </thead>
            ...
            @else
                <p class="p-8 text-center text-sm text-slate-500">{{ __('لا توجد محاولات اختبار.') }}</p>
            @endif
        </div>
    </section>
    ...
    <section class="rounded-2xl bg-white border border-slate-200 shadow-lg overflow-hidden">
        <div class="px-5 py-4 border-b border-slate-200">
            <h2 class="text-lg font-black text-slate-900">{{ __('الشهادات') }}</h2>
        </div>
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-slate-200 text-right text-sm">
                <thead class="bg-slate-50 text-xs font-bold text-slate-600">
                    <tr>
                        <th class="px-4 py-3">{{ __('العنوان / الكورس') }}</th>
                        <th class="px-4 py-3">{{ __('تاريخ الإصدار') }}</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100">
                    @foreach($certificates as $cert)
                        <tr class="hover:bg-slate-50">
                            <td class="px-4 py-3">{{ $cert->title ?? $cert->course_name ?? __('شهادة #') . $cert->id }}</td>
