...
<head>
    ...
    <meta name="description" content="{{ __('public.help_meta_description', ['brand' => $brand]) }}">
    ...
    <div class="edu-container-full relative z-10">
        <div class="edu-courses-inner text-center reveal">
            <nav class="edu-breadcrumb justify-center mb-4" aria-label="???? ??????">
                ...
                <span class="text-slate-800 font-semibold">{{ __('public.help_page_title') }}</span>
            </nav>
            <span class="edu-badge mb-4"><i class="fas fa-life-ring"></i> {{ __('public.help_hero_badge') }}</span>
            ...
            <div class="edu-hero-actions mt-6 justify-center">
                <a href="{{ route('public.faq') }}" class="edu-btn-primary">
    ...
    <div class="edu-container-full">
        <div class="edu-courses-inner">
            <div class="flex items-center gap-3 mb-6 reveal">
                ...
                <h2 class="text-xl font-bold text-slate-900">{{ __('public.help_start_title') }}</h2>
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-5 reveal s1">
                <a href="{{ route('public.faq') }}" class="edu-card edu-help-hub-card">
                    ...
                    <h3 class="text-lg font-bold text-slate-900 mb-2">{{ __('public.help_card_faq_title') }}</h3>
                    <p class="text-slate-600 text-sm leading-relaxed">{{ __('public.help_card_faq_desc') }}</p>
                </a>
                <a href="{{ route('public.contact') }}" class="edu-card edu-help-hub-card">
                    ...
                    <h3 class="text-lg font-bold text-slate-900 mb-2">{{ __('public.help_card_contact_title') }}</h3>
                    <p class="text-slate-600 text-sm leading-relaxed">{{ __('public.help_card_contact_desc') }}</p>
                </a>
                <a href="{{ route('public.courses') }}" class="edu-card edu-help-hub-card">
                    ...
                    <h3 class="text-lg font-bold text-slate-900 mb-2">{{ __('public.help_card_courses_title') }}</h3>
                    <p class="text-slate-600 text-sm leading-relaxed">{{ __('public.help_card_courses_desc') }}</p>
                </a>
            </div>
            ...
            <div class="flex items-center gap-3 mt-12 mb-6 reveal">
                ...
                <h2 class="text-xl font-bold text-slate-900">{{ __('public.help_topics_title') }}</h2>
            </div>
            <div class="edu-card !p-0 overflow-hidden reveal s1">
                @foreach(range(1, 5) as $i)
                <a href="{{ $topicLinks[$i - 1] }}" class="edu-help-topic">
                    <div class="flex items-start gap-4">
                        <span class="w-11 h-11 rounded-xl flex items-center justify-center shrink-0 text-white text-base"
                              style="background:{{ $i % 2 === 0 ? 'var(--edu-accent)' : 'var(--edu-primary)' }}">
                            <i class="fas fa-{{ $topicIcons[$i - 1] }}"></i>
                        </span>
                        <div class="flex-1 min-w-0 text-start">
                            <h3 class="text-base font-bold text-slate-900 mb-1">{{ __('public.help_topic_'.$i.'_title') }}</h3>
                            <p class="text-slate-600 text-sm leading-relaxed">{{ __('public.help_topic_'.$i.'_desc') }}</p>
                        </div>
            ...
            <div class="flex items-center gap-3 mt-12 mb-6 reveal">
                <span class="w-10 h-1 rounded-full shrink-0" style="background:linear-gradient(90deg,var(--edu-primary),var(--edu-accent))"></span>
                <h2 class="text-xl font-bold text-slate-900">{{ __('public.help_steps_title') }}</h2>
            </div>
            <div class="relative max-w-2xl mx-auto ps-10 sm:ps-12 border-s-2 border-[var(--edu-primary)]/20 reveal s2">
                ...
                <div class="relative pb-8 last:pb-0">
                    <span class="edu-help-step-num" style="background:{{ $step % 2 === 1 ? 'var(--edu-primary)' : 'var(--edu-accent)' }}">{{ $step }}</span>
                    <div class="edu-card px-5 py-4">
                        <p class="font-bold text-slate-900 mb-1">{{ __('public.help_step_'.$step.'_title') }}</p>
                        <p class="text-sm text-slate-600 leading-relaxed">{{ __('public.help_step_'.$step.'_desc') }}</p>
                    </div>
                </div>
                @endforeach
            </div>
    ...
    <div class="edu-container-full">
        <div class="edu-courses-inner reveal">
            <div class="edu-cta-wrap px-8 py-10 lg:py-12 text-center text-white">
                <h2 class="text-2xl sm:text-3xl font-bold mb-3">{{ __('public.help_cta_title') }}</h2>
                ...
                <div class="flex flex-col sm:flex-row gap-4 justify-center">
                    <a href="{{ route('public.contact') }}" class="edu-btn-white !text-[var(--edu-primary)]">
                        ...
                        {{ __('public.help_cta_btn') }}
                    </a>
                    <a href="{{ route('public.faq') }}" class="edu-btn-ghost-light">
                        <i class="fas fa-circle-question"></i>
                        {{ __('public.faq_page_title') }}
                    </a>
