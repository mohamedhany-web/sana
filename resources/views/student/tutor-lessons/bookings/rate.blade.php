...
<div class="sd-page w-full pb-8 max-w-md">
    <form method="post" action="{{ route('student.tutor-lessons.bookings.rate.store',$booking) }}" class="sd-panel sd-form">
        ...
        <div class="sd-panel-body space-y-4">@csrf
            <div><label>{{ __('التقييم (1-5)') }}</label><input type="number" name="rating" min="1" max="5" required></div>
            <div><label>{{ __('تعليق') }}</label><textarea name="comment" rows="3"></textarea></div>
            <button type="submit" class="sd-btn-primary w-full">{{ __('إرسال') }}</button>
        </div>
    </form>
</div>
@endsection
