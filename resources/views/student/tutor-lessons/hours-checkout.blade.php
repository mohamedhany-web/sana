...
@section('title', __('شراء ساعات — ').$plan['label'])
...
<div class="sd-page space-y-6 pb-8 w-full">
    <div class="sd-hero">
        <div class="sd-hero-main relative z-[1]">
            <p class="text-xs font-bold sd-tag mb-2">{{ __('إتمام الشراء') }}</p>
            <h1 class="font-heading text-2xl sm:text-3xl font-black text-slate-800 leading-tight">
                {{ $plan['label'] ?? __('باقة الساعات') }}
            </h1>
            <p class="text-slate-600 text-sm mt-2 max-w-2xl">
                {{ __('حوّل المبلغ ثم ارفع صورة الإيصال. بعد موافقة الأدمن تُضاف') }} <strong>{{ $hours }}</strong> {{ __('ساعة إلى رصيدك.') }}
            </p>
            <div class="flex flex-wrap gap-2 mt-4">
                <a href="{{ route('student.tutor-lessons.hours') }}" class="sd-btn-outline">
                    <i class="fas fa-arrow-right text-xs"></i> {{ __('العودة للساعات') }}
                </a>
            </div>
        </div>
        <div class="sd-motivation">
            <p class="font-bold text-sm">{{ __('المبلغ المطلوب') }}</p>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <div class="xl:col-span-1 space-y-4">
            <div class="sd-panel">
                <div class="sd-panel-head"><h2 class="font-heading font-bold text-slate-800 m-0">{{ __('ملخص') }}</h2></div>
                <div class="sd-panel-body space-y-2 text-sm">
                    <p class="m-0 flex justify-between gap-2"><span class="text-slate-500">{{ __('الباقة') }}</span><strong>{{ $plan['label'] }}</strong></p>
                    <p class="m-0 flex justify-between gap-2"><span class="text-slate-500">{{ __('الساعات') }}</span><strong>{{ $hours }}</strong></p>
                    <p class="m-0 flex justify-between gap-2"><span class="text-slate-500">{{ __('السعر') }}</span><strong>{{ number_format((float) $plan['price'], 0) }} {{ $currency }}</strong></p>
                </div>
            </div>

            @if($wallets->isNotEmpty())
                <div class="sd-panel">
                    <div class="sd-panel-head"><h2 class="font-heading font-bold text-slate-800 m-0">{{ __('حسابات التحويل') }}</h2></div>
        ...
        <div class="xl:col-span-2">
            <form method="post" action="{{ route('student.tutor-lessons.hours.purchase', $planKey) }}" enctype="multipart/form-data" class="sd-panel sd-form" data-turbo="false">
                <div class="sd-panel-head">
                    <h2 class="font-heading font-bold text-slate-800 m-0">{{ __('رفع إيصال الدفع') }}</h2>
                </div>
                <div class="sd-panel-body space-y-5">
                    ...
                    <div>
                        <label class="mb-2">{{ __('طريقة الدفع *') }}</label>
                        <select name="payment_method" id="payment_method" required class="w-full">
                            ...
                            <option value="wallet" @selected(old('payment_method', $wallets->isNotEmpty() ? 'wallet' : 'bank_transfer') === 'wallet')>{{ __('محفظة / إنستاباي') }}</option>
                        </select>
                    </div>

                    <div id="wallet_id_wrap" class="{{ old('payment_method', $wallets->isNotEmpty() ? 'wallet' : 'bank_transfer') === 'wallet' ? '' : 'hidden' }}">
                        <label class="mb-2">{{ __('المحفظة التي حوّلت إليها *') }}</label>
                    ...
                    <div>
                        <label class="mb-2">{{ __('صورة إيصال التحويل *') }}</label>
                    ...
                    <div>
                        <label class="mb-2">{{ __('ملاحظات (اختياري)') }}</label>
