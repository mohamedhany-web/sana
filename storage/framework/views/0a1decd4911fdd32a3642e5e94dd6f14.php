...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div class="rounded-2xl bg-white border border-slate-200 p-5 shadow-sm">
            ...
            <p class="text-3xl font-bold text-slate-900"><?php echo e(number_format($stats['avg_lesson_hours'] ?? 0, 1)); ?></p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <div class="rounded-2xl bg-white border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div class="p-4 space-y-2 max-h-[520px] overflow-y-auto">
                <?php $__currentLoopData = $planUsage; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $plan): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <a href="<?php echo e(route('admin.students-control.paid-features.show', $plan['key'])); ?>" class="flex items-center justify-between p-3 rounded-xl border border-slate-200 hover:border-violet-300 hover:bg-violet-50/40 transition-colors">
                        <div class="min-w-0">
                            <p class="text-sm font-semibold text-slate-900 truncate"><?php echo e($plan['label']); ?></p>
                            <p class="text-xs text-slate-500 font-mono"><?php echo e($plan['key']); ?></p>
                        </div>
                        <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold bg-violet-100 text-violet-700"><?php echo e(number_format($plan['count'])); ?> طالب</span>
                    </a>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                <a href="<?php echo e(route('admin.students-control.paid-features.show', 'custom')); ?>" class="flex items-center justify-between p-3 rounded-xl border border-dashed border-violet-200 hover:bg-violet-50/40 transition-colors">
                    <p class="text-sm font-semibold text-violet-900">باقات مخصصة</p>
                    <span class="text-xs font-bold text-violet-700"><?php echo e(number_format($customPlanStats['students_count'] ?? 0)); ?> طالب</span>
                </a>
            </div>
        </div>
