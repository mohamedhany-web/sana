...
<div class="space-y-6">
    <div class="flex items-center gap-3">
        <a href="<?php echo e(route('admin.live-sessions.show', $liveSession)); ?>" class="p-2 rounded-lg hover:bg-slate-100 text-slate-500 transition-colors"><i class="fas fa-arrow-right"></i></a>
        <h1 class="text-2xl font-bold text-slate-800"><i class="fas fa-edit text-amber-500 ml-2"></i>تعديل جلسة البث</h1>
    </div>

    <form method="POST" action="<?php echo e(route('admin.live-sessions.update', $liveSession)); ?>" class="bg-white rounded-xl border border-slate-200 p-6 space-y-5">
        ...
        <div class="grid md:grid-cols-2 gap-5">
            <div class="md:col-span-2">
                ...
                <input type="text" name="title" value="<?php echo e(old('title', $liveSession->title)); ?>" required class="w-full rounded-lg border-slate-300">
...
unset($__errorArgs, $__bag); ?>
            ...
            <div>
                ...
                <input type="datetime-local" name="scheduled_at" value="<?php echo e(old('scheduled_at', $liveSession->scheduled_at?->format('Y-m-d\TH:i'))); ?>" required class="w-full rounded-lg border-slate-300">
            </div>
            <div>
            ...
            <div>
                ...
                <input type="number" name="max_participants" value="<?php echo e(old('max_participants', $liveSession->max_participants)); ?>" min="2" max="1000" class="w-full rounded-lg border-slate-300">
            </div>
            <div>
                <label class="block text-sm font-semibold text-slate-700 mb-1">كلمة مرور</label>
                <input type="text" name="password" value="<?php echo e(old('password', $liveSession->password)); ?>" class="w-full rounded-lg border-slate-300">
            </div>
            <div class="md:col-span-2">
                <label class="block text-sm font-semibold text-slate-700 mb-1">وصف الجلسة</label>
                <textarea name="description" rows="3" class="w-full rounded-lg border-slate-300"><?php echo e(old('description', $liveSession->description)); ?></textarea>
            </div>
        </div>
        ...
        <div class="border-t border-slate-200 pt-5">
            ...
            <div class="grid sm:grid-cols-2 md:grid-cols-3 gap-3">
                ...
                <label class="flex items-center gap-2 p-3 bg-slate-50 rounded-lg cursor-pointer">
                    ...
                    <span class="text-sm text-slate-700"><?php echo e($chk['label']); ?></span>
                </label>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </div>
        </div>
        ...
        <div class="flex items-center gap-3 pt-2">
            ...
            <a href="<?php echo e(route('admin.live-sessions.show', $liveSession)); ?>" class="px-6 py-2.5 bg-slate-200 text-slate-700 rounded-xl font-medium hover:bg-slate-300 transition-colors">إلغاء</a>
        </div>
    </form>
</div>
