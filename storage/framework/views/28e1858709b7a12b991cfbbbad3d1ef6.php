...
<div class="w-full px-4 sm:px-6 lg:px-8 py-6 space-y-6">
    ...
    <div class="bg-white rounded-xl p-5 border border-gray-200 shadow-sm">
        <div class="flex flex-col sm:flex-row sm:items-start gap-4">
            ...
            <div class="min-w-0 flex-1">
                <h1 class="text-xl sm:text-2xl font-bold text-gray-900 mb-1"><?php echo e($label); ?></h1>
                <p class="text-sm text-gray-500 leading-relaxed"><?php echo e($description); ?></p>
    ...
    <?php if(in_array(($feature ?? ''), ['ai_tools', 'full_ai_suite'], true) && Route::has('student.ai-usages.index') && auth()->user()->canAccessStudentAiUsages()): ?>
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-end">
            <a href="<?php echo e(route('student.ai-usages.index')); ?>" class="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl border border-sky-200 bg-sky-50 text-sky-800 text-sm font-semibold shadow-sm hover:bg-sky-100 transition-colors">
    ...
    <?php if($requiresCourseSelection && $courses->isEmpty()): ?>
        <div class="bg-white rounded-xl border border-dashed border-gray-300 p-8 text-center shadow-sm">
            ...
            <p class="text-sm text-gray-600 max-w-xl mx-auto"><?php echo e(__('student.full_ai_suite.course_empty')); ?></p>
    ...
    <?php else: ?>
        <div class="bg-white rounded-xl p-5 sm:p-6 border border-gray-200 shadow-sm space-y-5">
            <div>
                <h2 class="text-lg font-bold text-gray-900"><?php echo e(__('student.full_ai_suite.form_card_title')); ?></h2>
                <p class="text-sm text-gray-500 mt-1"><?php echo e($pageHint); ?></p>
            </div>
            ...
            <form action="<?php echo e(route('student.features.full-ai-suite.preview')); ?>" method="POST" class="space-y-5">
                ...
                <div class="space-y-5" x-data="{ questionType: <?php echo json_encode(old('question_type') ?: array_key_first($qTypes), 15, 512) ?>, len: <?php echo e(strlen(old('question', ''))); ?> }">
                    ...
                    <div>
                        ...
                        <textarea name="question" id="question" rows="6" required minlength="10" maxlength="4000"
                            ...
                            class="w-full rounded-xl border border-gray-200 bg-white text-gray-900 px-4 py-3 text-sm leading-relaxed shadow-sm focus:ring-2 focus:ring-sky-500/50 focus:border-sky-500 resize-y"><?php echo e(old('question')); ?></textarea>
                ...
                <div class="flex flex-col sm:flex-row sm:items-center gap-3 pt-1">
                    ...
                    <p class="text-xs text-gray-500 flex-1"><?php echo e(__('student.full_ai_suite.preview_hint_short')); ?></p>
                </div>
            </form>
        </div>
    <?php endif; ?>
    ...
    <?php if($preview && is_array($preview)): ?>
        <div class="bg-white rounded-xl p-5 border border-gray-200 shadow-sm space-y-3">
            <h2 class="text-lg font-bold text-gray-900"><?php echo e(__('student.full_ai_suite.result_title')); ?></h2>
            <p class="text-sm text-gray-500"><?php echo e(__('student.full_ai_suite.result_lead')); ?></p>

            <div class="rounded-xl border border-gray-100 bg-gray-50 p-4">
                <h3 class="text-sm font-semibold text-gray-800 mb-2"><?php echo e(__('student.full_ai_suite.custom_description_title')); ?></h3>
                <p class="text-sm text-gray-700 leading-relaxed whitespace-pre-wrap"><?php echo e(data_get($preview, 'context.option_description', '—')); ?></p>
            </div>
            ...
            <?php if($SanaAiText !== ''): ?>
                <div class="rounded-xl border border-emerald-200 bg-emerald-50/90 p-4">
                    ...
                    <div class="text-sm text-gray-900 whitespace-pre-wrap leading-relaxed"><?php echo e($SanaAiText); ?></div>
                </div>
            ...
            <?php if($gameHtmlUrl !== ''): ?>
                <div class="rounded-xl border border-gray-200 overflow-hidden">
                    <div class="flex flex-wrap items-center justify-between gap-2 px-4 py-3 bg-gray-50 border-b border-gray-200">
                        <h3 class="text-sm font-semibold text-gray-900"><?php echo e(__('student.full_ai_suite.educational_game_preview_title')); ?></h3>
                    ...
                    <?php if($gameStoragePath !== '' && Route::has('student.ai-usages.saved-games.store') && auth()->user()->canAccessStudentAiUsages()): ?>
                        <div class="px-4 py-4 bg-slate-50 border-t border-gray-200 space-y-3">
                            <p class="text-xs text-gray-600"><?php echo e(__('student.ai_usages.save_hint')); ?></p>
                            <form action="<?php echo e(route('student.ai-usages.saved-games.store')); ?>" method="POST" class="flex flex-col sm:flex-row sm:flex-wrap sm:items-end gap-3">
                                ...
                                <div class="flex-1 min-w-[200px]">
                                    <label for="saved_game_title" class="block text-xs font-semibold text-gray-700 mb-1"><?php echo e(__('student.ai_usages.title_optional')); ?></label>
                            ...
                            <?php if(Route::has('student.ai-usages.index') && auth()->user()->canAccessStudentAiUsages()): ?>
                                <a href="<?php echo e(route('student.ai-usages.index')); ?>" class="inline-flex items-center gap-2 text-sm font-semibold text-sky-600 hover:underline">
    ...
    <div class="flex justify-center sm:justify-start">
        <a href="<?php echo e(route('student.my-subscription')); ?>" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl border border-gray-200 bg-white text-gray-700 text-sm font-semibold hover:bg-gray-50 transition-colors shadow-sm">
