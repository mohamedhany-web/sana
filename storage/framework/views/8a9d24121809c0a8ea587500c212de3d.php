...
<header class="ta-nav">
    <div class="edu-container flex items-center justify-between gap-4 py-3">
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="<?php echo e(route('staff.login')); ?>" class="text-sm font-bold text-[var(--edu-primary)] no-underline hover:underline">دخول المعلمين</a>
        </div>
    </div>
</header>
...
<div class="ta-layout">
    ...
    <main class="ta-form-col">
        <?php if($applyStepErrors->isNotEmpty()): ?>
        <div class="ta-alert-err">
            <?php if($resumeStep === 8): ?>
                <p class="font-bold mb-2">يرجى إعادة رفع الملفات والمرفقات — المتصفح لا يحفظها تلقائياً بعد الخطأ.</p>
            <?php endif; ?>
            <?php $__currentLoopData = $applyStepErrors->all(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $err): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php echo e($err); ?><?php if(!$loop->last): ?><br><?php endif; ?> <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </div>
        <?php endif; ?>
        ...
        <form action="<?php echo e(route('tutor.apply.store')); ?>" method="POST" enctype="multipart/form-data" @submit.prevent="onSubmit" id="tutorApplyForm" novalidate>
            <?php echo csrf_field(); ?>
            ...
            <?php if(!empty($useDynamicForm) && $formSteps->isNotEmpty()): ?>
                <?php echo $__env->make('tutor.partials.apply-steps-dynamic', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
            <?php else: ?>
                <?php echo $__env->make('tutor.partials.apply-steps', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
            <?php endif; ?>
        </form>
...
function tutorApplyWizard() {
    ...
    const serverResumeStep = <?php echo e((int) $resumeStep); ?>;
    const hasServerErrors = <?php echo e($applyStepErrors->isNotEmpty() ? 'true' : 'false'); ?>;
    const tips = {
        ...
        8: { title: 'فيديو الشرح', text: 'ارفع ملفاً حتى <?php echo e(\App\Services\TutorApplicationFormService::videoMaxMb()); ?> ميجا، أو استخدم رابط YouTube / Drive.' },
        10: { title: 'الالتزام', text: 'بنود السرية والقنوات الرسمية إلزامية.' },
    };
    return {
        step: serverResumeStep,
        totalSteps: <?php echo e((int) ($totalSteps ?? 11)); ?>,
        submitting: false,
        stepError: '',
        draftRestored: false,
        _draftTimer: null,
        ...
        validateStep(stepNum) {
            ...
            if (fileInput || linkInput) {
                ...
                var maxBytes = <?php echo e(\App\Services\TutorApplicationFormService::videoMaxMb()); ?> * 1024 * 1024;
                ...
                if (hasFile && fileInput.files[0].size > maxBytes) {
                    ...
                    if (stepNum === this.step) {
                        this.stepError = 'حجم الفيديو يتجاوز ' + <?php echo e(\App\Services\TutorApplicationFormService::videoMaxMb()); ?> + ' ميجابايت — استخدم رابطاً خارجياً.';
                    }
                }
                ...
                if (!hasFile && !linkVal) {
                    ...
                    if (stepNum === this.step) {
                        this.stepError = 'ارفع فيديو (حتى <?php echo e(\App\Services\TutorApplicationFormService::videoMaxMb()); ?> ميجا) أو أدخل رابطاً خارجياً.';
                    }
                }
