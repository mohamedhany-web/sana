...
<header class="stu-topbar flex items-center justify-between gap-2 sm:gap-3 px-3 sm:px-4 md:px-6 flex-shrink-0 sticky top-0 z-30">
    <div class="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
        ...
        <div class="min-w-0 flex-1">
            <p class="text-xs sm:text-sm font-bold text-teal-800 truncate sm:hidden"><?php echo e(__('parent.portal_title')); ?></p>
            <div class="hidden sm:block max-w-md min-w-0">
                <p class="text-sm font-bold text-teal-800 truncate"><?php echo e(__('parent.portal_title')); ?></p>
                <p class="text-xs text-slate-500 truncate hidden md:block"><?php echo e(__('parent.dashboard_subtitle')); ?></p>
            </div>
        </div>
    </div>
        ...
        <div class="relative" x-data="{ open: false }">
            <button type="button" @click="open = !open" class="stu-icon-btn relative" aria-label="<?php echo e(__('student.notifications')); ?>">
                ...
                <?php if($navUnreadCount > 0): ?>
                    <span class="absolute -top-1 -right-1 min-w-[18px] h-[18px] px-1 bg-teal-600 rounded-full text-[9px] font-bold text-white flex items-center justify-center border-2 border-white">
            ...
            <div x-show="open" @click.away="open = false" x-cloak x-transition
                 class="absolute end-0 mt-2 stu-dd stu-dd--menu z-50">
                <div class="px-4 py-3 border-b border-slate-100 flex items-center justify-between gap-2">
                    <h3 class="text-sm font-bold text-slate-900"><?php echo e(__('student.notifications')); ?></h3>
                    <a href="<?php echo e(route('notifications')); ?>" class="text-xs font-bold text-teal-700 hover:underline">
                ...
                <?php if($navRecentNotifications->isNotEmpty()): ?>
                    <div class="max-h-96 overflow-y-auto">
                        <?php $__currentLoopData = $navRecentNotifications; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $notification): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            ...
                            ?>
                            <a href="<?php echo e($notificationUrl); ?>" class="block px-4 py-3 border-b border-slate-50 hover:bg-teal-50/50 transition-colors no-underline text-inherit">
                                <div class="flex items-start gap-3">
                                    <span class="w-8 h-8 rounded-lg bg-teal-50 text-teal-700 flex items-center justify-center flex-shrink-0">
        ...
        <div class="relative" x-data="{ open: false }">
            ...
            <div x-show="open" @click.away="open = false" x-cloak x-transition
                ...
                <div class="p-1.5 space-y-0.5">
                    <a href="<?php echo e(route('parent.profile')); ?>" class="flex items-center gap-2.5 px-3 py-2 rounded-xl text-sm font-semibold text-slate-700 hover:bg-slate-50 no-underline">
                        <i class="fas fa-user text-slate-400 w-4 text-center text-xs"></i>
                        <?php echo e(__('parent.profile')); ?>

                    </a>
                    <a href="<?php echo e(route('parent.settings')); ?>" class="flex items-center gap-2.5 px-3 py-2 rounded-xl text-sm font-semibold text-slate-700 hover:bg-slate-50 no-underline">
                        <i class="fas fa-cog text-slate-400 w-4 text-center text-xs"></i>
                        <?php echo e(__('parent.settings')); ?>
...
<?php /**PATH C:\xampp\htdocs\sana\resources\views/layouts/partials/parent-app-header.blade.php ENDPATH**/ ?>