    ...
    <div class="rounded-2xl bg-white border border-slate-200 shadow-sm p-6 space-y-4">
        ...
        <?php if($meeting->ended_at): ?>
            ...
            <div class="rounded-xl border border-sky-200 bg-sky-50/70 p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                <div>
                    <p class="text-sm font-bold text-slate-800"><?php echo e($audioReportEnabled ? 'التسجيل والتقرير الصوتي' : 'تسجيل المحاضرة'); ?></p>
                    <?php if($hasAnyMedia): ?>
                        <?php if($hasVideo): ?>
                        ...
                        <?php endif; ?>
                    <?php else: ?>
                        <p class="text-xs text-slate-600 mt-1"><?php echo e($audioReportEnabled ? 'لا يوجد تسجيل أو تقرير صوتي مرفوع لهذا الاجتماع.' : 'لا يوجد تسجيل مرفوع لهذا الاجتماع.'); ?></p>
                    <?php endif; ?>
                </div>
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    <?php if($audioReportEnabled): ?>
                        ...
                        <?php endif; ?>
                    <?php endif; ?>
                    <?php if(!$meeting->recording_download_url && (!$audioReportEnabled || !$meeting->recording_audio_download_url) && $hasAnyMedia): ?>
            ...
            <div class="rounded-xl border border-violet-200 bg-violet-50/60 p-4 space-y-3">
                <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                    ...
                    <?php if($canGenerateReport && !($activeAiReport ?? null)): ?>
                        <form method="POST" action="<?php echo e(route($rp.'classroom.ai-report', $meeting)); ?>" class="shrink-0" onsubmit="return confirm('<?php echo e($audioReportEnabled ? 'سيتم إرسال رابط التسجيل/التقرير الصوتي لإنشاء تقرير نصي عن المحاضرة. هل تريد المتابعة؟' : 'سيتم إرسال رابط تسجيل المحاضرة لإنشاء تقرير نصي. هل تريد المتابعة؟'); ?>');">
                            <?php echo csrf_field(); ?>
                    ...
                    <?php elseif(! $canGenerateReport): ?>
                        <p class="text-xs text-amber-800 shrink-0 max-w-xs"><?php echo e($audioReportEnabled ? 'ارفع التسجيل أو التقرير الصوتي أولاً حتى يظهر زر إنشاء التقرير.' : 'ارفع تسجيل المحاضرة أولاً حتى يظهر زر إنشاء التقرير.'); ?></p>
                    <?php endif; ?>
                </div>
