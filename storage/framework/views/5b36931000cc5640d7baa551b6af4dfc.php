...
<head>
    ...
    <title><?php echo $__env->yieldContent('title', __('auth.dashboard')); ?> - <?php echo e($platformName ?? config('brand.name', config('app.name'))); ?></title>
        ...
        tailwind.config = {
            theme: {
                extend: {
                    ...
                    colors: {
                        brand: {
                            DEFAULT: '<?php echo e(config('brand.colors.blue')); ?>',
                            light: '<?php echo e(config('brand.colors.blue_dark')); ?>',
                            dark: '<?php echo e(config('brand.colors.purple')); ?>',
                        }
                    }
                }
            }
...
<body class="font-body"
      data-admin-path="<?php echo e(request()->path()); ?>"
      data-admin-route="<?php echo e(Route::currentRouteName()); ?>"
      x-data="{ 
          sidebarOpen: false,
    ...
    <div class="content-wrapper">
        ...
        <header class="top-navbar top-navbar--brand sticky top-0 z-40 flex items-center px-4 lg:px-7 gap-3">
            ...
            <?php $mobileAdminLogoUrl = $adminPanelLogoUrl ?? $platformLogoUrl ?? null; ?>
            <?php if(! empty($mobileAdminLogoUrl)): ?>
            <a href="<?php echo e(route('admin.dashboard')); ?>" class="flex items-center shrink-0 lg:hidden sidebar-brand-icon sidebar-brand-icon--sm" title="<?php echo e($platformName ?? config('brand.name', config('app.name'))); ?>">
                <img src="<?php echo e($mobileAdminLogoUrl); ?>" alt="<?php echo e($platformName ?? config('brand.name', config('app.name'))); ?>" width="36" height="36" class="sidebar-brand-icon__img">
            </a>
            <?php endif; ?>

            <!-- Page title -->
            <div class="flex-1 min-w-0">
                <p class="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-0.5 hidden sm:block"><?php echo e($platformName ?? config('brand.name', config('app.name'))); ?> · لوحة الإدارة</p>
            ...
            <div class="flex items-center gap-2">
                ...
                <div class="relative"
                     x-data="adminNavNotifications(<?php echo e(\Illuminate\Support\Js::from($adminNavBellConfig ?? ['unread' => 0, 'items' => [], 'pollUrl' => route('admin.api.nav-notifications')])); ?>)"
                ...
                <a href="<?php echo e($adminNavSettingsUrl); ?>" title="إعدادات النظام" aria-label="إعدادات النظام" class="admin-nav-icon-btn flex items-center justify-center active:scale-95">
                ...
                <div class="relative" x-data="{ open: false }" @click.outside="open = false">
                    <button @click.stop="open = !open" class="flex items-center gap-2.5 px-2.5 py-2 rounded-xl hover:bg-slate-50 transition-all active:scale-[0.98]" :aria-expanded="open">
                        <?php if(auth()->user()->profile_image): ?>
                            ...
                            <div class="w-9 h-9 rounded-xl hidden flex items-center justify-center text-white font-bold text-sm" style="background: linear-gradient(135deg, var(--admin-primary), var(--admin-purple));"><?php echo e(mb_substr(auth()->user()->name, 0, 1)); ?></div>
                        <?php else: ?>
                    ...
                    <div x-show="open" x-cloak
                        ...
                        <div class="py-1.5">
                            ...
                            <a href="<?php echo e($adminNavSettingsUrl); ?>" class="flex items-center gap-3 px-4 py-2.5 text-[13px] text-slate-600 hover:bg-slate-50 hover:text-slate-800 transition-colors">
                                <i class="fas fa-cog w-4 text-slate-400 text-xs"></i> إعدادات النظام
                            </a>
                        </div>
                        <div class="border-t border-slate-100 py-1.5">
                            <form method="POST" action="<?php echo e(route('logout')); ?>" data-turbo="false">
                                <?php echo csrf_field(); ?>
    ...
    <?php echo $__env->make('partials.admin-shell-nav', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
</body>
</html>
<?php /**PATH C:\xampp\htdocs\sana\resources\views\layouts\admin.blade.php ENDPATH**/ ?>