...
<div class="space-y-6">
    ...
    <div class="bg-slate-100 rounded-xl border border-slate-200 px-4 py-2 text-sm text-slate-600">
        ...
        <strong>النطاق الافتراضي الحالي لـ Jitsi:</strong> <code class="bg-slate-200 px-1.5 py-0.5 rounded"><?php echo e($defaultJitsiDomain); ?></code>
    </div>
    <?php endif; ?>
    ...
    <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div class="bg-white rounded-xl border <?php echo e($isDefault ? 'border-cyan-500 ring-1 ring-cyan-500/30' : 'border-slate-200'); ?> p-5 space-y-4">
            <div class="flex items-start justify-between">
                <div>
                    <h3 class="font-bold text-slate-800"><?php echo e($server->name); ?></h3>
                    <p class="text-sm text-slate-400 font-mono mt-0.5"><?php echo e($server->domain); ?></p>
                    <?php if($isDefault): ?><span class="inline-block mt-1 px-2 py-0.5 rounded bg-cyan-100 text-cyan-700 text-xs font-medium">نطاق افتراضي</span><?php endif; ?>
                </div>
            ...
            <div class="space-y-2 text-sm">
                <div class="flex justify-between"><span class="text-slate-500">النوع:</span><span class="font-medium text-slate-700"><?php echo e(ucfirst($server->provider)); ?></span></div>
                <?php if($server->ip_address): ?>
                <div class="flex justify-between"><span class="text-slate-500">IP:</span><span class="font-mono text-xs text-slate-600"><?php echo e($server->ip_address); ?></span></div>
                <?php endif; ?>
                <div class="flex justify-between"><span class="text-slate-500">الجلسات النشطة:</span><span class="font-bold text-slate-700"><?php echo e($server->active_sessions_count); ?></span></div>
            ...
            <div class="flex flex-wrap items-center gap-2 pt-2 border-t border-slate-100">
                <form method="POST" action="<?php echo e(route('admin.live-servers.test-connection', $server)); ?>" class="inline"><?php echo csrf_field(); ?><button type="submit" class="inline-flex items-center gap-1 px-3 py-1.5 bg-cyan-50 text-cyan-600 rounded-lg text-xs font-medium hover:bg-cyan-100"> <i class="fas fa-plug"></i> اختبار الاتصال</button></form>
                <?php if($server->status === 'active' && !$isDefault): ?>
                <form method="POST" action="<?php echo e(route('admin.live-servers.set-default', $server)); ?>" class="inline"><?php echo csrf_field(); ?><button type="submit" class="inline-flex items-center gap-1 px-3 py-1.5 bg-violet-50 text-violet-600 rounded-lg text-xs font-medium hover:bg-violet-100"> <i class="fas fa-star"></i> استخدام كنطاق افتراضي</button></form>
                <?php endif; ?>
                <?php if(!empty($server->config['ssh_username'] ?? null)): ?>
                <a href="<?php echo e(route('admin.live-servers.ssh-browse', $server)); ?>" class="px-3 py-1.5 bg-emerald-50 text-emerald-600 rounded-lg text-xs font-medium hover:bg-emerald-100">ملفات</a>
                <?php endif; ?>
                <a href="<?php echo e(route('admin.live-servers.edit', $server)); ?>" class="px-3 py-1.5 bg-slate-100 text-slate-600 rounded-lg text-xs font-medium hover:bg-slate-200">تعديل</a>
                <form method="POST" action="<?php echo e(route('admin.live-servers.toggle-status', $server)); ?>" class="inline"><?php echo csrf_field(); ?><button class="px-3 py-1.5 bg-slate-100 text-slate-600 rounded-lg text-xs font-medium hover:bg-slate-200"><?php echo e($server->status === 'active' ? 'إيقاف' : 'تفعيل'); ?></button></form>
                <form method="POST" action="<?php echo e(route('admin.live-servers.destroy', $server)); ?>" class="inline" onsubmit="return confirm('حذف السيرفر؟')"><?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?><button class="px-3 py-1.5 bg-red-50 text-red-500 rounded-lg text-xs font-medium hover:bg-red-100">حذف</button></form>
            </div>
        </div>
