...
<div class="w-full px-4 sm:px-6 lg:px-8 py-6 space-y-6">
    ...
    <div class="rounded-2xl bg-white border border-slate-200 shadow-sm p-6">
        <div class="flex items-center justify-between gap-3">
            <div>
                <h1 class="text-xl font-black text-slate-900"><?php echo e($ticket->subject); ?></h1>
                <p class="text-xs text-slate-500 mt-1">
                    التصنيف: <?php echo e($ticket->inquiryCategory->name ?? '—'); ?>

                    <span class="mx-1">|</span>
                    الحالة: <?php echo e($ticket->statusLabel()); ?>

                    <span class="mx-1">|</span>
                    الأولوية: <?php echo e($ticket->priorityLabel()); ?>
    ...
    <?php if(!in_array($ticket->status, ['resolved', 'closed'], true)): ?>
        <form action="<?php echo e(route('student.support.reply', $ticket)); ?>" method="POST" class="rounded-2xl bg-white border border-slate-200 shadow-sm p-5 space-y-3">
            <?php echo csrf_field(); ?>
            <label class="block text-sm font-semibold text-slate-700">إضافة رد</label>
            <textarea name="message" rows="4" class="w-full px-3 py-2 rounded-lg border border-slate-200 bg-white text-slate-800"><?php echo e(old('message')); ?></textarea>
