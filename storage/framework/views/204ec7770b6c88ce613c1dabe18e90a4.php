...
?>
<article class="<?php echo e($widthClass); ?> <?php echo e($fluid ? '' : 'shrink-0'); ?> <?php echo e($cardClass); ?> p-6 sm:p-8 flex flex-col h-full dark:bg-slate-800 dark:border-slate-700">
    <?php if($t->isImageType() && $t->publicImageUrl()): ?>
        <div class="w-full aspect-[4/3] max-h-[14rem] rounded-xl overflow-hidden bg-slate-50 dark:bg-slate-700/50 mb-5 flex items-center justify-center">
            <img src="<?php echo e($t->publicImageUrl()); ?>" alt="" class="max-h-full max-w-full object-contain" loading="lazy" decoding="async">
        </div>
    <?php else: ?>
        <span class="w-11 h-11 rounded-xl bg-blue-50 text-[var(--edu-primary)] dark:bg-blue-900/30 dark:text-blue-300 flex items-center justify-center text-lg mb-5 shrink-0">
            <i class="fas fa-quote-right"></i>
        </span>
    <?php endif; ?>
    <div class="flex flex-col flex-1">
        <?php if($t->body): ?>
            <p class="text-sm leading-8 flex-1 text-slate-600 dark:text-slate-300">
        ...
        <div class="mt-5 pt-4 border-t border-slate-100 dark:border-slate-600">
            <?php if($t->author_name): ?>
                <p class="font-bold text-sm text-slate-900 dark:text-white"><?php echo e($t->author_name); ?></p>
            <?php endif; ?>
            <?php if($t->role_label): ?>
                <p class="text-xs mt-1 text-slate-500 dark:text-slate-400"><?php echo e($t->role_label); ?></p>
            <?php endif; ?>
