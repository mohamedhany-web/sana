...
<div class="space-y-6 w-full max-w-full" x-data="personalBrandingForm(<?php echo \Illuminate\Support\Js::from(old('skills', $profile->skills))->toHtml() ?>, <?php echo \Illuminate\Support\Js::from($skillsPreview)->toHtml() ?>)">
    ...
    <div class="rounded-2xl bg-white dark:bg-slate-800/95 border border-slate-200 dark:border-slate-700 shadow-sm p-4 sm:p-5 ring-2 <?php echo e($statusStyles['ring']); ?>">
        <div class="flex flex-col sm:flex-row sm:items-center gap-3 sm:justify-between">
            <div class="flex items-start gap-3 min-w-0">
                ...
                <div class="min-w-0">
                    <p class="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide m-0 mb-1"><?php echo e(__('instructor.status_label')); ?></p>
            ...
            <p class="text-xs text-slate-500 dark:text-slate-400 m-0 sm:text-left max-w-md"><?php echo e(__('instructor.personal_branding_desc')); ?></p>
        </div>
    </div>
                ...
                <div class="p-5 sm:p-6 border-b border-slate-200 dark:border-slate-700">
                    ...
                    <div class="mt-4 flex flex-col sm:flex-row gap-5 items-center">
                        ...
                        <label class="flex-1 w-full cursor-pointer rounded-2xl border-2 border-dashed border-slate-300 dark:border-slate-600 hover:border-indigo-400 bg-slate-50 dark:bg-slate-900/50 hover:bg-indigo-50/40 p-5 text-center transition-colors">
                            ...
                            <p class="text-sm font-semibold text-slate-700 dark:text-slate-200 m-0"><?php echo e(__('instructor.choose_image_label')); ?></p>
...
unset($__errorArgs, $__bag); ?>
                ...
                <div class="p-5 sm:p-6 border-b border-slate-200 dark:border-slate-700 space-y-4">
                    ...
                    <div>
                        <label for="pb-headline" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2"><?php echo e(__('instructor.intro_title')); ?></label>
...
unset($__errorArgs, $__bag); ?>
                    ...
                    <div>
                        <label for="pb-bio" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2"><?php echo e(__('instructor.bio')); ?></label>
                        <textarea id="pb-bio" name="bio" rows="5" placeholder="<?php echo e(__('instructor.bio_placeholder')); ?>"
                                  class="w-full px-4 py-2.5 border border-slate-200 dark:border-slate-600 rounded-xl bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-slate-100 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 text-sm"><?php echo e(old('bio', $profile->bio)); ?></textarea>
                        <?php $__errorArgs = ['bio'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
...
unset($__errorArgs, $__bag); ?>
                ...
                <div class="p-5 sm:p-6 border-b border-slate-200 dark:border-slate-700">
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-2 mb-3 m-0"><?php echo e(__('instructor.experience_placeholder')); ?></p>
                    <textarea name="experience" rows="8" placeholder="سطر لكل خبرة أو فقرة..."
                              class="w-full px-4 py-2.5 border border-slate-200 dark:border-slate-600 rounded-xl bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-slate-100 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 text-sm font-mono leading-relaxed"><?php echo e(old('experience', $profile->experience)); ?></textarea>
...
unset($__errorArgs, $__bag); ?>
                ...
                <div class="p-5 sm:p-6">
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-2 mb-3 m-0"><?php echo e(__('instructor.skills_hint')); ?></p>
...
unset($__errorArgs, $__bag); ?>
            ...
            <form method="POST" action="<?php echo e(route('instructor.personal-branding.submit')); ?>" class="mt-4">
                ...
                <div class="rounded-2xl border border-amber-200 dark:border-amber-800/60 bg-amber-50/80 dark:bg-amber-950/30 p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div class="min-w-0">
                        <p class="text-sm font-bold text-amber-900 dark:text-amber-200 m-0"><?php echo e(__('instructor.submit_for_review')); ?></p>
                    ...
                    <button type="submit" <?php if(!$canSubmit): echo 'disabled'; endif; ?>
                            ...
                            <?php echo e($canSubmit ? 'bg-amber-500 hover:bg-amber-600 text-white cursor-pointer' : 'bg-slate-300 dark:bg-slate-600 text-slate-500 cursor-not-allowed'); ?>">
        ...
        <aside class="lg:col-span-4 space-y-4">
            ...
            <div class="rounded-2xl bg-white dark:bg-slate-800/95 border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
                ...
                <div class="p-4">
                    <div class="flex items-center gap-3 mb-3">
                        ...
                        <div class="min-w-0">
                            <p class="text-sm font-bold text-slate-900 dark:text-white m-0 truncate"><?php echo e($user->name); ?></p>
                            <p class="text-xs text-indigo-600 dark:text-indigo-400 m-0 truncate"><?php echo e($profile->headline ?: '—'); ?></p>
                        </div>
                    </div>
                    <p class="text-xs text-slate-600 dark:text-slate-400 line-clamp-4 m-0 mb-3"><?php echo e(Str::limit($profile->bio ?: 'النبذة التعريفية...', 160)); ?></p>
                    <div class="flex flex-wrap gap-1.5">
                        <?php $__empty_1 = true; $__currentLoopData = array_slice($skillsPreview, 0, 6); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $skill): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <span class="text-[10px] font-semibold px-2 py-0.5 rounded-md bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300"><?php echo e($skill); ?></span>
            ...
            <div class="rounded-2xl bg-white dark:bg-slate-800/95 border border-slate-200 dark:border-slate-700 shadow-sm p-5">
                ...
                <ul class="space-y-2.5 m-0 p-0 list-none text-sm">
                    ...
                    <li class="flex items-center gap-2 <?php echo e($check['ok'] ? 'text-emerald-700 dark:text-emerald-400' : 'text-slate-500 dark:text-slate-400'); ?>">
