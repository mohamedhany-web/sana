...
<header class="ins-topbar flex items-center justify-between gap-3 px-4 md:px-6 flex-shrink-0 sticky top-0 z-30">
        ...
        <div class="min-w-0 hidden sm:block">
            <?php if(!$onDashboard): ?>
                <p class="ins-page-breadcrumb mb-0.5">
                    <a href="<?php echo e(route('dashboard')); ?>" class="text-[#283593] hover:underline no-underline"><?php echo e(__('instructor.dashboard')); ?></a>
                    <span class="mx-1 opacity-40">/</span>
                    <span><?php echo e($pageTitle); ?></span>
                </p>
            <?php endif; ?>
            <h1 class="ins-page-title m-0"><?php echo e($onDashboard ? __('instructor.dashboard') : $pageTitle); ?></h1>
        </div>

        <form action="<?php echo e(route('instructor.courses.index')); ?>" method="get" class="ins-topbar-search hidden md:flex min-w-0 ms-auto max-w-xs lg:max-w-sm">
    ...
    <div class="flex items-center gap-2 flex-shrink-0">
        <?php if(Route::has('instructor.classroom.index') && $currentUser->hasSubscriptionFeature('classroom_access')): ?>
        <a href="<?php echo e(route('instructor.classroom.create')); ?>" class="hidden sm:inline-flex items-center gap-2 px-3.5 py-2 rounded-xl bg-[#283593] hover:bg-[#1F2A7A] text-white text-xs font-bold transition-colors no-underline shadow-sm shadow-indigo-900/15">
        ...
        <div class="relative" x-data="{ open: false }">
            <button type="button" @click="open = !open" class="ins-icon-btn relative" aria-label="<?php echo e($isRtl ? 'الإشعارات' : 'Notifications'); ?>">
            ...
            <div x-show="open" @click.away="open = false" x-cloak x-transition class="absolute left-0 mt-2 w-80 ins-dd z-50">
                <div class="px-4 py-3 border-b border-slate-100 flex items-center justify-between gap-2">
                    <h3 class="text-sm font-bold text-slate-900"><?php echo e($isRtl ? 'الإشعارات' : 'Notifications'); ?></h3>
                    <?php if(Route::has('notifications')): ?>
                        <a href="<?php echo e(route('notifications')); ?>" class="text-xs font-bold text-[#283593] hover:underline">
                            <?php echo e($isRtl ? 'عرض الكل' : 'View all'); ?>
                ...
                <?php if($navRecentNotifications->isNotEmpty()): ?>
                    <div class="max-h-96 overflow-y-auto">
                        <?php $__currentLoopData = $navRecentNotifications; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $notification): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            ...
                            ?>
                            <a href="<?php echo e($notificationUrl); ?>" class="block px-4 py-3 border-b border-slate-50 hover:bg-[#FFE5F7]/40 transition-colors no-underline text-inherit">
                                <div class="flex items-start gap-3">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        <div class="flex items-center justify-between gap-2">
                                            <p class="text-sm font-bold text-slate-800 truncate"><?php echo e($notification->title); ?></p>
        ...
        <div class="relative" x-data="{ open: false }">
            <button type="button" @click="open = !open" class="ins-user-chip">
                ...
                <span class="hidden lg:block text-sm font-bold text-slate-700 max-w-[110px] truncate"><?php echo e($currentUser->name); ?></span>
            ...
            <div x-show="open" @click.away="open = false" x-cloak x-transition class="absolute left-0 mt-2 w-56 ins-dd z-50">
                <div class="px-4 py-3 border-b border-slate-100">
                    <p class="text-sm font-bold text-slate-900 truncate"><?php echo e($currentUser->name); ?></p>
                    <p class="text-xs text-slate-500 truncate mt-0.5"><?php echo e($currentUser->email ?? '—'); ?></p>
                    <span class="inline-flex mt-2 text-[10px] font-bold uppercase tracking-wide px-2 py-0.5 rounded-md bg-[#FFE5F7] text-[#283593]"><?php echo e(__('instructor.instructor_role')); ?></span>
                </div>
                <div class="p-1.5 space-y-0.5">
                    <a href="<?php echo e(route('instructor.profile')); ?>" class="flex items-center gap-2.5 px-3 py-2 rounded-xl text-sm font-semibold text-slate-700 hover:bg-slate-50 no-underline">
                        <i class="fas fa-user text-slate-400 w-4 text-center text-xs"></i>
                        <?php echo e(__('instructor.profile')); ?>

                    </a>
                    <a href="<?php echo e(route('settings')); ?>" class="flex items-center gap-2.5 px-3 py-2 rounded-xl text-sm font-semibold text-slate-700 hover:bg-slate-50 no-underline">
                        <i class="fas fa-cog text-slate-400 w-4 text-center text-xs"></i>
                        <?php echo e(__('instructor.settings')); ?>

                    </a>
                    <a href="<?php echo e(route('home')); ?>" target="_blank" rel="noopener" class="flex items-center gap-2.5 px-3 py-2 rounded-xl text-sm font-semibold text-slate-700 hover:bg-slate-50 no-underline">
                        <i class="fas fa-external-link-alt text-slate-400 w-4 text-center text-xs"></i>
                        <?php echo e($isRtl ? 'الموقع العام' : 'Public site'); ?>
                    ...
                    <form method="POST" action="<?php echo e(route('logout')); ?>">
                        ...
                        <button type="submit" class="w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-sm font-bold text-red-600 hover:bg-red-50 text-right">
                            ...
                            <?php echo e(__('instructor.logout')); ?>
...
<?php /**PATH C:\xampp\htdocs\sana\resources\views\layouts\partials\instructor-app-header.blade.php ENDPATH**/ ?>