...
<div class="space-y-6">
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-xl font-bold text-slate-900">
                        استهلاك المشترك
                        <?php if($subscription->subscriberIsStudent()): ?> (طالب) <?php elseif($subscription->subscriberIsInstructor()): ?> (مدرب) <?php endif; ?>
                    </h1>
    ...
    <div class="rounded-2xl bg-white border border-violet-200 shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-violet-100 bg-violet-50 flex items-center justify-between">
            ...
            <a href="<?php echo e(route('admin.tutor-lessons.bookings')); ?>" class="text-xs font-bold text-violet-700 underline">كل الحجوزات</a>
        </div>
        <div class="p-6 grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
            <div class="rounded-xl bg-violet-50 p-4">
                ...
                <p class="text-2xl font-black"><?php echo e((int) ($learningProfile->lesson_hours_quota ?? $subscription->tutorLessonHoursFromLimits() ?? 0)); ?></p>
            </div>
            <div class="rounded-xl bg-amber-50 p-4">
                <p class="text-xs text-amber-800">مستهلكة</p>
                <p class="text-2xl font-black"><?php echo e((int) ($learningProfile->lesson_hours_used ?? 0)); ?></p>
            </div>
            <div class="rounded-xl bg-emerald-50 p-4">
                <p class="text-xs text-emerald-800">عدد الحجوزات</p>
                <p class="text-2xl font-black"><?php echo e($tutorBookingsCount ?? 0); ?></p>
            </div>
        </div>
        <?php if($tutorBookings->isNotEmpty()): ?>
        <div class="px-6 pb-6 space-y-2">
            <?php $__currentLoopData = $tutorBookings; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $b): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="flex justify-between text-sm border border-slate-100 rounded-lg p-3">
                    <span><?php echo e($b->scheduled_at?->format('Y-m-d H:i')); ?> — <?php echo e($b->instructor?->name); ?></span>
                    <span class="font-bold"><?php echo e($b->status); ?></span>
                </div>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </div>
        <?php endif; ?>
    ...
    <div class="rounded-2xl bg-white border border-sky-200 shadow-sm p-6">
        <p class="text-sm text-slate-700"><?php echo e(__('platform.classroom_sessions')); ?> هذا الشهر: <strong class="text-sky-800"><?php echo e($classroomMeetingsMonth); ?></strong></p>
    </div>
    <?php endif; ?>
