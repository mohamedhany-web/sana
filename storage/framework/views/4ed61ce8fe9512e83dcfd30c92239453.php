...
<div class="space-y-6">
    ...
    <form method="POST" action="<?php echo e(route('admin.live-servers.update', $liveServer)); ?>" class="bg-white rounded-xl border border-slate-200 p-6 space-y-5">
        <?php echo csrf_field(); ?> <?php echo method_field('PUT'); ?>
        <div class="grid md:grid-cols-2 gap-5">
            <div class="md:col-span-2">
                <label class="block text-sm font-semibold text-slate-700 mb-1">اسم السيرفر</label>
                <input type="text" name="name" value="<?php echo e(old('name', $liveServer->name)); ?>" required class="w-full rounded-lg border-slate-300">
            </div>
            <div>
                <label class="block text-sm font-semibold text-slate-700 mb-1">النطاق</label>
                <input type="text" name="domain" value="<?php echo e(old('domain', $liveServer->domain)); ?>" required class="w-full rounded-lg border-slate-300">
            </div>
            <div>
            ...
            <div>
                ...
                <input type="text" name="ip_address" value="<?php echo e(old('ip_address', $liveServer->ip_address)); ?>" class="w-full rounded-lg border-slate-300">
            </div>
            <div>
                <label class="block text-sm font-semibold text-slate-700 mb-1">الحد الأقصى</label>
                <input type="number" name="max_participants" value="<?php echo e(old('max_participants', $liveServer->max_participants)); ?>" min="2" max="10000" required class="w-full rounded-lg border-slate-300">
            </div>
            <div class="md:col-span-2">
                <label class="block text-sm font-semibold text-slate-700 mb-1">رابط لوحة التحكم بالسيرفر (اختياري)</label>
                <input type="url" name="control_panel_url" value="<?php echo e(old('control_panel_url', $liveServer->control_panel_url)); ?>" class="w-full rounded-lg border-slate-300" placeholder="https://panel.example.com">
            ...
            <div class="md:col-span-2 border-t border-slate-200 pt-5 mt-2">
                ...
                <div class="grid md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="ssh_host" value="<?php echo e(old('ssh_host', $liveServer->config['ssh_host'] ?? $liveServer->ip_address ?? $liveServer->domain)); ?>" class="w-full rounded-lg border-slate-300" placeholder="IP أو النطاق">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-600 mb-1">المنفذ (Port)</label>
                        <input type="number" name="ssh_port" value="<?php echo e(old('ssh_port', $liveServer->config['ssh_port'] ?? 22)); ?>" min="1" max="65535" class="w-full rounded-lg border-slate-300">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-600 mb-1">اسم المستخدم</label>
                        <input type="text" name="ssh_username" value="<?php echo e(old('ssh_username', $liveServer->config['ssh_username'] ?? '')); ?>" class="w-full rounded-lg border-slate-300" placeholder="root أو ubuntu">
                    </div>
                    <div>
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="notes" rows="3" class="w-full rounded-lg border-slate-300"><?php echo e(old('notes', $liveServer->notes)); ?></textarea>
            </div>
        </div>
