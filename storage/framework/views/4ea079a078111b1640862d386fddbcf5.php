            ...
            <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div class="min-w-0 flex-1">
                    <nav class="flex flex-wrap items-center gap-2 text-xs sm:text-sm text-gray-500 mb-3">
                        <a href="<?php echo e(route('student.assignments.index')); ?>" class="inline-flex items-center gap-1.5 font-semibold text-[#283593] hover:text-[#FB5607] transition-colors">
                            <i class="fas fa-arrow-right text-[10px] sm:text-xs"></i>
                            واجباتي
                        </a>
                        <span class="text-gray-300" aria-hidden="true">/</span>
                        <span class="truncate text-gray-600"><?php echo e($courseTitle); ?></span>
                    </nav>
                    <h1 class="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 font-heading leading-tight"><?php echo e($assignment->title); ?></h1>
                ...
                <div class="flex flex-wrap items-center gap-2 shrink-0">
                    <?php if($assignment->due_date): ?>
                        <div class="inline-flex items-center gap-2 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs sm:text-sm">
                            ...
                            <div>
                                ...
                                <span class="text-amber-800"><?php echo e($assignment->due_date->timezone(config('app.timezone'))->format('Y-m-d H:i')); ?></span>
                    ...
                    <div class="inline-flex items-center gap-2 rounded-xl border border-gray-200 bg-gray-50 px-3 py-2 text-xs sm:text-sm">
                        ...
                        <span class="text-gray-700">الدرجة العظمى: <strong class="text-gray-900"><?php echo e($assignment->max_score); ?></strong></span>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
                ...
                <div class="p-4 sm:p-6 space-y-4">
                    <?php if($assignment->description): ?>
                        <div class="prose prose-sm max-w-none text-gray-700 leading-relaxed"><?php echo nl2br(e($assignment->description)); ?></div>
                    <?php else: ?>
                    ...
                    <?php if($assignment->instructions): ?>
                        <div class="rounded-xl bg-slate-50 border border-slate-200 p-4 sm:p-5">
                            ...
                            <div class="text-sm text-slate-800 whitespace-pre-wrap leading-relaxed"><?php echo e($assignment->instructions); ?></div>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
                                ...
                                <?php if($url): ?>
                                    <li class="text-sm rounded-xl border border-sky-100 overflow-hidden bg-sky-50/30">
                                        <?php if($isImg): ?>
                                            ...
                                            <div class="p-3">
                                                <a href="<?php echo e($url); ?>" target="_blank" rel="noopener" class="text-sky-700 font-semibold hover:underline text-xs"><?php echo e($label); ?></a>
                                            </div>
                                        <?php else: ?>
                                            <a href="<?php echo e($url); ?>" target="_blank" rel="noopener" class="flex items-center gap-3 p-4 hover:bg-sky-50/80 transition-colors">
                                                ...
                                                <span class="font-semibold text-sky-800 min-w-0 break-words"><?php echo e($label); ?></span>
                                            </a>
                                        <?php endif; ?>
                                    </li>
                                <?php endif; ?>
                ...
                <div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
                    ...
                    <div class="p-4 sm:p-5 space-y-3">
                        <p class="text-sm text-gray-600">
                            ...
                            <?php elseif($submission->status === 'graded'): ?>
                                ...
                                <?php if($submission->score !== null): ?>
                                    <span class="text-gray-700"> — الدرجة: <?php echo e($submission->score); ?> / <?php echo e($assignment->max_score); ?></span>
                                <?php endif; ?>
                        ...
                        <?php if($submission->content): ?>
                            <div class="rounded-xl border border-gray-200 bg-gray-50 p-4 text-sm text-gray-800 whitespace-pre-wrap max-h-64 overflow-y-auto"><?php echo e($submission->content); ?></div>
                        <?php endif; ?>
                            ...
                            <div>
                                ...
                                <ul class="space-y-1.5">
                                    <?php $__currentLoopData = $submission->attachments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $att): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        ...
                                        <?php if($fileUrl): ?>
                                            <li>
                                                <a href="<?php echo e($fileUrl); ?>" target="_blank" rel="noopener" class="inline-flex items-center gap-2 text-[#283593] hover:text-[#FB5607] text-sm font-medium">
                        ...
                        <?php if(!empty($canDeleteSubmission)): ?>
                            <form action="<?php echo e(route('student.assignments.submission.destroy', $assignment)); ?>" method="post" class="pt-2 border-t border-gray-200" onsubmit="return confirm('سيتم حذف التسليم بالكامل ومرفقاته من التخزين. هل أنت متأكد؟');">
                                <?php echo csrf_field(); ?>
                    ...
                    <div class="p-4 sm:p-5">
                        <form id="assignment-submit-form" action="<?php echo e(route('student.assignments.submit', $assignment)); ?>" method="post" enctype="multipart/form-data" class="space-y-4">
                            ...
                            <div>
                                ...
                                <textarea name="content" rows="8" class="w-full rounded-xl border border-gray-300 bg-white px-3 py-2.5 text-sm text-gray-900 focus:ring-2 focus:ring-[#283593]/30 focus:border-[#283593] transition-shadow"><?php echo e(old('content', ($submission && $submission->status === 'returned') ? ($submission->content ?? '') : '')); ?></textarea>
