...
<div class="w-full px-4 sm:px-6 lg:px-8 py-6 space-y-6">
    ...
    <div class="rounded-2xl bg-white border border-slate-200 shadow-lg p-6">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            ...
            <a href="<?php echo e(route($rp.'classroom.create')); ?>" class="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-red-500 hover:bg-red-600 text-white text-sm font-bold shadow-lg shadow-red-500/30">
    ...
    <div class="rounded-2xl bg-white border border-slate-200 shadow-sm p-5">
        ...
        <div class="flex flex-wrap items-center gap-3">
            <a href="<?php echo e(route($rp.'classroom.whiteboard')); ?>" target="_blank" rel="noopener noreferrer"
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div class="rounded-xl border border-slate-200 bg-white p-4">
            ...
            <p class="text-xl font-bold text-slate-800"><?php echo e(number_format($stats['total'])); ?></p>
        </div>
        <div class="rounded-xl border border-slate-200 bg-white p-4">
            <p class="text-xs text-slate-500">اجتماعات مباشرة</p>
            <p class="text-xl font-bold text-rose-600"><?php echo e(number_format($stats['live'])); ?></p>
        </div>
        <div class="rounded-xl border border-slate-200 bg-white p-4">
            <p class="text-xs text-slate-500">الحد الشهري / المستخدم</p>
            <p class="text-xl font-bold text-slate-800"><?php echo e(number_format($usedMeetingsThisMonth)); ?> / <?php echo e(number_format($limits['classroom_meetings_per_month'])); ?></p>
        </div>
        <div class="rounded-xl border border-slate-200 bg-white p-4">
            <p class="text-xs text-slate-500">المتبقي هذا الشهر</p>
            <p class="text-xl font-bold <?php echo e($remainingMeetingsThisMonth > 0 ? 'text-emerald-600' : 'text-rose-600'); ?>"><?php echo e(number_format($remainingMeetingsThisMonth)); ?></p>
        </div>
    </div>

    <div class="rounded-2xl bg-white border border-slate-200 shadow-sm overflow-hidden">
        <div class="px-6 py-4 bg-slate-50 border-b border-slate-200">
            <form method="GET" action="<?php echo e(route($rp.'classroom.index')); ?>" class="flex flex-wrap items-center gap-2">
                ...
                <?php $__currentLoopData = ['all' => 'الكل', 'live' => 'مباشر', 'scheduled' => 'مجدول', 'ended' => 'منتهي']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $k => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <button type="submit" name="status" value="<?php echo e($k); ?>" class="px-3 py-1.5 rounded-lg text-xs font-semibold <?php echo e($status === $k ? 'bg-sky-600 text-white' : 'bg-slate-100 text-slate-700'); ?>">
                        <?php echo e($label); ?>
                ...
                <tbody class="divide-y divide-slate-100">
                    <?php $__empty_1 = true; $__currentLoopData = $meetings; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $m): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        ...
                        <tr class="hover:bg-slate-50/60">
                            <td class="px-4 py-3">
                                <p class="text-sm font-semibold text-slate-900"><?php echo e($m->title ?: 'اجتماع بدون عنوان'); ?></p>
                            ...
                            <td class="px-4 py-3 text-sm font-mono text-slate-700"><?php echo e($m->code); ?></td>
                            <td class="px-4 py-3">
                                <?php if($m->isLive()): ?>
                            ...
                            <td class="px-4 py-3 text-sm">
                                <button type="button" onclick="navigator.clipboard.writeText('<?php echo e($joinUrl); ?>'); this.textContent='تم النسخ'; setTimeout(()=>this.textContent='نسخ', 1000)" class="px-2.5 py-1 rounded-md bg-slate-100 text-slate-700 text-xs font-semibold">نسخ</button>
                            </td>
                            <td class="px-4 py-3 text-sm">
                                <div class="flex items-center gap-2">
                                    <a href="<?php echo e(route($rp.'classroom.show', $m)); ?>" class="text-sky-600 hover:underline">عرض</a>
                                    <a href="<?php echo e(route($rp.'classroom.edit', $m)); ?>" class="text-amber-600 hover:underline">تعديل</a>
                                    <?php if(!$m->started_at && !$m->ended_at): ?>
                                        <form action="<?php echo e(route($rp.'classroom.start-meeting', $m)); ?>" method="POST" class="inline"><?php echo csrf_field(); ?><button class="text-emerald-600 hover:underline">بدء</button></form>
                                    <?php elseif($m->isLive()): ?>
                                        <a href="<?php echo e(route($rp.'classroom.room', $m)); ?>" class="text-rose-600 hover:underline">دخول</a>
        ...
        <div class="px-6 py-4 border-t border-slate-200"><?php echo e($meetings->links()); ?></div>
    </div>
</div>
